...
This section provides a roadmap to the \textbf{nontrivial formal
results} in this paper.

\textbf{Theorems and Identifiability Guarantees:}
...
\bottomrule\noalign{}
\endlastfoot
\textbf{Theorem 1} & \(K_0\)-IRT Exact Correspondence &
\(K_0 = \tanh(a(\theta-b)/2)\) with \textbf{formal derivation} & Formal
Results \\
\textbf{Theorem 2} & \(K_1\)-Phi Correspondence & \(K_1 = \phi\) under
binary \(\text{State}_0/\text{Claim}_1\) & Formal Results \\
\textbf{Theorem 3} & \(K_0\) Identifiability &
\(\text{Var}(b_i) > 0 \Rightarrow K_0\) identifiable & Formal Results \\
\textbf{Theorem 4} & \(K_1\) Identifiability & Variability conditions
for \(K_1\) identification & Formal Results \\
\textbf{Theorem 5} & \(K_2\) Identifiability & ICC-based identifiability
conditions & Formal Results \\
\textbf{Theorem 6} & Pipeline Identifiability & Joint identifiability of
\((K_0, K_1, K_2)\) \textbf{without circularity} & Formal Results \\
\end{longtable}
...
\item
  ...
  standard conditions
\item
  \textbf{Correspondences are derived}, not heuristic: Theorem 1
  provides exact derivation from 2PL-IRT
\end{itemize}
...
\hypertarget{formal-results-theorems-and-identifiability-guarantees}{%
\subsection{Formal Results: Theorems and Identifiability
Guarantees}\label{formal-results-theorems-and-identifiability-guarantees}}

This section provides mathematical theorems and propositions that
establish the formal foundations of the \(K\) framework. These results
transform the framework from definitional to substantive by proving key
properties and relationships.

\begin{quote}
\textbf{Summary of Contributions}: This section contains \textbf{6
theorems, 1 lemma, and 5 falsifiable predictions}. The main
identifiability results are: - \textbf{Theorem 3}: \(K_0\)
identifiability under item variance conditions - \textbf{Theorem 4}:
\(K_1\) identifiability given \(K_0\) and claim variability -
\textbf{Theorem 6}: Joint \((K_0, K_1, K_2)\) pipeline identifiability
without circularity
...
\begin{quote}
\textbf{Note on Contribution Type}: The theorems below primarily
integrate existing mathematical results (IRT, Signal Detection Theory,
ICC) into the \(K\) framework. The novelty lies not in the underlying
mathematics but in the unified integration that enables systematic
metacognition analysis. Theorems 1-5 establish that the \(K\) framework
inherits desirable properties (identifiability, proper scoring) from its
component theories.
\end{quote}

\hypertarget{core-theorems}{%
\subsubsection{Core Theorems}\label{core-theorems}}

\hypertarget{theorem-1-k_0-irt-correspondence}{%
\paragraph{\texorpdfstring{Theorem 1: \(K_0\)-IRT
Correspondence}{Theorem 1: K\_0-IRT Correspondence}}\label{theorem-1-k_0-irt-correspondence}}

\textbf{Theorem 1} (\(K_0\)-IRT Exact Correspondence):

\textbf{Assumptions}: - \textbf{(A1)} Item response follows the
...
\textbf{Statement}: Under (A1)-(A2), for each item \(i\):
\[K_0^{(i)} = \tanh\left(\frac{a_i(\theta - b_i)}{2}\right)\]

\textbf{Proof}: Starting from the 2PL probability:
...
\hypertarget{theorem-2-k_1-phi-correspondence}{%
\paragraph{\texorpdfstring{Theorem 2: \(K_1\)-Phi
Correspondence}{Theorem 2: K\_1-Phi Correspondence}}\label{theorem-2-k_1-phi-correspondence}}

\textbf{Statement}: When \(\text{State}_0\) and \(\text{Claim}_1\) are
both binary, \(K_1\) equals the Phi coefficient.

\textbf{Theorem 2} (\(K_1\)-Phi Correspondence): Let
...
\hypertarget{identifiability-theorems}{%
\subsubsection{Identifiability
Theorems}\label{identifiability-theorems}}

\hypertarget{theorem-3-k_0-identifiability}{%
\paragraph{\texorpdfstring{Theorem 3: \(K_0\)
Identifiability}{Theorem 3: K\_0 Identifiability}}\label{theorem-3-k_0-identifiability}}

\textbf{Theorem 3} (\(K_0\) Identifiability): \(K_0\) is identifiable
from response data iff item difficulties have positive variance.
...
\hypertarget{theorem-4-k_1-identifiability}{%
\paragraph{\texorpdfstring{Theorem 4: \(K_1\)
Identifiability}{Theorem 4: K\_1 Identifiability}}\label{theorem-4-k_1-identifiability}}

\textbf{Theorem 4} (\(K_1\) Identifiability): Given \(K_0\) estimates
and \(\text{Claim}_1\) observations, \(K_1\) is identifiable iff: 1.
\(P(K_0 = +1) > 0\) and \(P(K_0 \leq 0) > 0\) (variability in
\(\text{State}_0\)) 2. \(\text{Claim}_1\) is non-degenerate:
\(P(\text{Claim}_1 = \text{"I know"}) \in (0, 1)\)

\textbf{Proof}: The Phi coefficient \(\phi\) is undefined when any
marginal frequency is zero (denominator becomes zero). Condition 1
ensures both rows of the \(2 \times 2\) table are populated. Condition 2
ensures both columns are populated. With all cells potentially non-zero,
\(\phi\) is a well-defined estimator of alignment. \(\blacksquare\)

\begin{center}\rule{0.5\linewidth}{0.5pt}\end{center}

\hypertarget{theorem-5-k_2-identifiability}{%
\paragraph{\texorpdfstring{Theorem 5: \(K_2\)
Identifiability}{Theorem 5: K\_2 Identifiability}}\label{theorem-5-k_2-identifiability}}

\textbf{Theorem 5} (\(K_2\) Identifiability): Let
...
\textbf{Proof}: The ICC is undefined when within-subject or
...
\hypertarget{theorem-6-recursive-pipeline-identifiability}{%
\paragraph{Theorem 6: Recursive Pipeline
Identifiability}\label{theorem-6-recursive-pipeline-identifiability}}

\begin{quote}
\textbf{Critical Note}: This theorem addresses the potential circularity
...
\textbf{Theorem 6} (Pipeline Identifiability): Under the following
...
\(K_0 \to K_1 \to K_2\) ensures no feedback loops. By Theorem 3
...
The paper establishes the following formal results (detailed in ``Formal
Results: Theorems and Identifiability Guarantees''):
...
\bottomrule\noalign{}
\endlastfoot
\textbf{Theorem 1} & \(K_0\)-IRT correspondence:
\(K_0 = \tanh(a(\theta-b)/2)\) & Formal Results \\
\textbf{Theorem 2} & \(K_1\)-Phi correspondence under binary conditions
& Formal Results \\
\textbf{Theorem 3} & \(K_0\) identifiability:
\(\text{Var}(b_i) > 0 \Rightarrow K_0\) identifiable & Formal Results \\
\textbf{Theorem 4} & \(K_1\) identifiability given \(K_0\) and claim
variability & Formal Results \\
\textbf{Theorem 5} & ICC reliability conditions for stable \(K\)
measurement & Formal Results \\
\textbf{Theorem 6} & Pipeline identifiability: \((K_0, K_1, K_2)\)
jointly identifiable without circularity & Formal Results \\
\textbf{Lemma 3} & \(\hat{K}\) sufficiency: any monotone
anchor-preserving function yields equivalent ordinal results & Formal
Results \\
\end{longtable}

\textbf{Key Identifiability Guarantees}: - The framework provides
sufficient conditions under which parameters are recoverable from data -
No circularity exists in the recursive measurement pipeline (Theorem 6)
- The specific form of \(\hat{K}\) is sufficient but not unique (Lemma
3)
...
\begin{quote}
\textbf{Note}: This remark clarifies the \emph{conceptual relationship}
between \(K_1\) and meta-d'. We do \textbf{not} claim mathematical
equivalence; rather, these measures capture related but distinct aspects
of metacognition.
\end{quote}

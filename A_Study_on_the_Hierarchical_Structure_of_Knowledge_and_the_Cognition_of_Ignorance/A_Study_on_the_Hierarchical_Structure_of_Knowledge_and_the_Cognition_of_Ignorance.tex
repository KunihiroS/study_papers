...
\hypertarget{relationship-to-formal-epistemic-logic}{%
\subsubsection{Relationship to Formal Epistemic
Logic}\label{relationship-to-formal-epistemic-logic}}

Classical epistemic logic (modal logic S5) assumes \textbf{idealized
agents} with perfect introspection. The following table shows how our
framework relates to the standard S5 axioms:
...
\bottomrule\noalign{}
\endlastfoot
\textbf{T} & Truth & \(K\phi \to \phi\) & Satisfied: \(K_0 = +1\)
implies correctness \\
\textbf{4} & Positive Introspection & \(K\phi \to KK\phi\) &
\textbf{Violated}: \(K_0 = +1\) does NOT imply \(K_1 = +1\) \\
...
\textbf{Misconception} & Not modeled (\(K\phi\) only for true \(\phi\))
& Explicitly modeled (\(K = -1\)) \\
\end{longtable}

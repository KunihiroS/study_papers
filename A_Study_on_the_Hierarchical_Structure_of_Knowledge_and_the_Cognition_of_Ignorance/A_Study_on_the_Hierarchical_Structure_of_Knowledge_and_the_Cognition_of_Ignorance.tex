...
\item
  ...
  established metrics (IRT, meta-d', ECE)
\end{enumerate}
...
\hypertarget{proposition-1b-k_1-ece-approximate-relationship}{%
\paragraph{\texorpdfstring{Proposition 1b: \(K_1\)-ECE Approximate
Relationship}{Proposition 1b: K\_1-ECE Approximate Relationship}}\label{proposition-1b-k_1-ece-approximate-relationship}}

\textbf{Proposition 1b} (\(K_1\)-ECE Approximate Relationship):

Under specific conditions, \(K_1\) can be approximated from Expected
Calibration Error (ECE).

\textbf{Statement}: Under fixed binning with \(B\) bins and large sample
size \(N\): \[K_1 \approx 1 - 2 \cdot \text{ECE}\]
...
\begin{longtable}[]{@{}
  ...
  >{\raggedright\arraybackslash}p{(\columnwidth - 4\tabcolsep) * \real{0.1940}}
  >{\raggedright\arraybackslash}p{(\columnwidth - 4\tabcolsep) * \real{0.7015}}@{}}
\toprule\noalign{}
\begin{minipage}[b]{\linewidth}\raggedright
Issue
\end{minipage} & \begin{minipage}[b]{\linewidth}\raggedright
Description
\end{minipage} & \begin{minipage}[b]{\linewidth}\raggedright
Impact on \(K_1 \approx 1 - 2 \cdot \text{ECE}\)
\end{minipage} \\
\midrule\noalign{}
\endhead
\bottomrule\noalign{}
\endlastfoot
\textbf{Binning-dependent} & Different \(B\) \(\to\) different ECE &
Same data can yield different \(K_1\) estimates \\
\textbf{Sample-size dependent} & Small \(N\) \(\to\) high variance in
ECE & Unreliable \(K_1\) estimates \\
\textbf{Always non-negative} & ECE \(\geq 0\) by definition & Cannot
distinguish over- from under-confidence \\
\textbf{Aggregate only} & ECE is computed across all items & No
item-level \(K_1^{(i)}\) \\
\end{longtable}

\textbf{Scope of Validity}: - Approximately valid for \(B \geq 10\),
\(N \geq 500\), balanced confidence distribution - Breaks down for small
samples, extreme base rates, or heavily skewed confidence

\textbf{Recommendation}: - Use \(K_1 = \phi\) (Phi coefficient) as the
\textbf{primary} measure for the \(K\) framework - Use
\(K_1 \approx 1 - 2 \cdot \text{ECE}\) only for \textbf{cross-study
comparison} where ECE is the standard metric - Always report binning
scheme \(B\) and sample size \(N\) when using ECE-based approximation -
Consider Brier score decomposition for more stable calibration
assessment

...
Where \textbf{Object} can be:
\begin{itemize}
    \item A proposition (e.g., ``The Earth is round'')
    \item A cognitive state (e.g., $K(x) \in [-1, 1]$)
    \item A metacognitive state (e.g., $K(K(x)) \in [-1, 1]$)
\end{itemize}

\textbf{Output Interpretation:}
\begin{itemize}
    \item $K(\text{object}) = 1$: The subject accurately recognizes the object.
    \item $K(\text{object}) = 0$: The subject does not recognize the object (ignorance).
    \item $K(\text{object}) = -1$: The subject misrecognizes the object (misconception).
\end{itemize}

\textbf{Key Insight:} The function $K$ has \textbf{consistent semantics} across all layers: ``How accurately does the subject recognize this object?''
...
Since \texttt{EpistemicState} is a subtype of \texttt{EpistemicObject}, we can apply $K$ recursively:
...
\begin{itemize}
    \item $K(x)$: Epistemic state (how accurately the subject recognizes $x$)
    \item $C$: Phenomenological confidence (how certain the subject feels)
\end{itemize}

These are \textbf{orthogonal dimensions}. A subject can have:
\begin{itemize}
    \item $K(x) = 0$ (ignorance) with $C = 1$ (high confidence) — Dunning-Kruger
    \item $K(x) = 1$ (knowledge) with $C = 0.5$ (moderate confidence) — Underconfidence
\end{itemize}

\subsection{Measurement Protocol}

\subsubsection{Step 1: Establish Ground Truth $T(x)$}
...
\subsubsection{Step 2: Measure $K(x)$ via Task Performance}

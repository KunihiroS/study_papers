...
\textbf{Statement}: Under the 2-Parameter Logistic (2PL) Item Response
Theory model, \(K_0\) is exactly determined by the latent ability
parameter \(\theta\).

\textbf{Theorem 1} (\(K_0\)-IRT Correspondence): Let
\((\theta, \{a_i, b_i\}_{i=1}^N)\) be a 2PL-IRT model with:
...
\textbf{Corollary 1.1} (Standardized Form): Under standardization
assumptions \(a = 2\) and \(b = 0\): \[K_0 \approx \tanh(\theta)\]
...
ensures both rows of the 2×2 table are populated. Condition 2 ensures
both columns are populated. With all cells potentially non-zero,
...
The Phi coefficient achieves its maximum when the 2×2 table has
deterministic structure. Given marginal constraint
...
\hypertarget{proposition-1-k_1-meta-d-approximate-relationship}{%
\paragraph{\texorpdfstring{Proposition 1: \(K_1\)-meta-d' Approximate
Relationship}{Proposition 1: K\_1-meta-d' Approximate Relationship}}\label{proposition-1-k_1-meta-d-approximate-relationship}}

\textbf{Proposition 1} (\(K_1\)-meta-d' Correspondence): Under Gaussian
Signal Detection Theory assumptions:
\[K_1 \approx \tanh\left(\frac{\text{meta-d}'}{2}\right)\]

\textbf{Scope of Validity}: This approximation holds when: 1. Confidence
ratings are normally distributed conditioned on correctness 2. Type-2
criteria are symmetric around the Type-1 criterion 3. meta-d' is
computed using standard maximum-likelihood methods

\textbf{When the Approximation Fails}: - Non-Gaussian confidence
distributions - Asymmetric Type-2 criteria - Small sample sizes where
meta-d' estimation is unreliable

\textbf{Note}: This is an approximate correspondence, not an exact
equivalence. The relationship is useful for connecting \(K\) framework
results to the Type-2 SDT literature.
...
\textbf{Claim 1} (Framework Novelty --- Survey-Based): Based on our
survey of existing approaches, the \(K\) framework is the only approach

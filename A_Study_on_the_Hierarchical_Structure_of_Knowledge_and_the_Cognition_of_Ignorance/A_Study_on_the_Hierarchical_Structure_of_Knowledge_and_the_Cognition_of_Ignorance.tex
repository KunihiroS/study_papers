...
\hypertarget{theorem-1-kux2080-irt-correspondence}{%
\paragraph{Theorem 1: K₀-IRT
Correspondence}\label{theorem-1-kux2080-irt-correspondence}}
...
\textbf{Theorem 1} (K₀-IRT Correspondence): Let
...
\hypertarget{theorem-2-kux2081-phi-correspondence}{%
\paragraph{Theorem 2: K₁-Phi
Correspondence}\label{theorem-2-kux2081-phi-correspondence}}

\textbf{Statement}: When State₀ and Claim₁ are both binary, \(K_1\)
equals the Phi coefficient.

\textbf{Theorem 2} (K₁-Phi Correspondence): Let State₀ ∈ \{correct,
incorrect\} and Claim₁ ∈ \{``I know'', ``I don't know''\} be binary
random variables. Define \(K_1 := \phi(\text{State}_0, \text{Claim}_1)\)
where \(\phi\) is the Phi coefficient:
\[\phi = \frac{n_{11} n_{00} - n_{10} n_{01}}{\sqrt{(n_{11}+n_{10})(n_{01}+n_{00})(n_{11}+n_{01})(n_{10}+n_{00})}}\]

Then: 1. \(K_1 \in [-1, 1]\) 2. \(K_1 = +1\) iff Claim₁ = State₀ for all
items (perfect alignment) 3. \(K_1 = -1\) iff Claim₁ = ¬State₀ for all
items (perfect anti-alignment) 4. \(K_1 = 0\) iff Claim₁ ⊥ State₀
...
\textbf{Property 2} (\(K_1 = +1\) iff perfect alignment): When Claim₁ =
State₀ for all items, \(n_{10} = n_{01} = 0\). Then:
...
Claim₁ = ¬State₀ for all items, \(n_{11} = n_{00} = 0\). Then:
...
\textbf{Extension to Ternary State₀}: When State₀ ∈ \{correct,
incorrect, absent\}, binarization is required for Phi. The recommended
strategy: - Positive: correct only - Negative: incorrect + absent
...
\hypertarget{theorem-3-kux2080-identifiability}{%
\paragraph{Theorem 3: K₀
Identifiability}\label{theorem-3-kux2080-identifiability}}

\textbf{Theorem 3} (K₀ Identifiability): \(K_0\) is identifiable from
response data iff item difficulties have positive variance.
...
\hypertarget{theorem-4-kux2081-identifiability}{%
\paragraph{Theorem 4: K₁
Identifiability}\label{theorem-4-kux2081-identifiability}}

\textbf{Theorem 4} (K₁ Identifiability): Given \(K_0\) estimates and
Claim₁ observations, \(K_1\) is identifiable iff: 1. \(P(K_0 = +1) > 0\)
and \(P(K_0 \leq 0) > 0\) (variability in State₀) 2. Claim₁ is
non-degenerate: \(P(\text{Claim}_1 = \text{"I know"}) \in (0, 1)\)
...
\hypertarget{theorem-5-kux2082-identifiability}{%
\paragraph{Theorem 5: K₂
Identifiability}\label{theorem-5-kux2082-identifiability}}

\textbf{Theorem 5} (K₂ Identifiability): Let
...
\textbf{Setup}: Elicit Claim₁ as a probability \(p \in [0, 1]\): ``What
is the probability that your answer is correct?''
...
\textbf{Implication for K₁}: When subjects are paid according to Brier
score, strategic responding is discouraged. The observed \(p\) reflects
genuine beliefs, ensuring that:
...
\hypertarget{lemma-1-scale-invariance-of-kux2080}{%
\paragraph{Lemma 1: Scale Invariance of
K₀}\label{lemma-1-scale-invariance-of-kux2080}}
...
\hypertarget{lemma-2-kux2081-under-class-imbalance}{%
\paragraph{Lemma 2: K₁ under Class
Imbalance}\label{lemma-2-kux2081-under-class-imbalance}}
...
All \(K_0 = +1\) cases have Claim₁ = ``I know'', and the remaining ``I
know'' claims are minimized. - The resulting maximum is:
...
\hypertarget{proposition-1-kux2081-meta-d-approximate-relationship}{%
\paragraph{Proposition 1: K₁-meta-d' Approximate
Relationship}\label{proposition-1-kux2081-meta-d-approximate-relationship}}

\textbf{Proposition 1} (K₁-meta-d' Correspondence): Under Gaussian
...
\bottomrule\noalign{}
\endlastfoot
\textbf{(a)} Per-item metacognitive classification & ✅ & ❌ & ❌ &
✅ \\
\textbf{(b)} Explicit modeling of ignorance (\(K=0\)) distinct from
misconception (\(K=-1\)) & ✅ & ❌ & ❌ & ❌ \\
\textbf{(c)} Recursive higher-order extension (\(K_2, K_3, ...\)) & ✅ &
❌ & ❌ & ❌ \\
\textbf{(d)} Unified anchor semantics across layers & ✅ & N/A & N/A &
N/A \\
\end{longtable}

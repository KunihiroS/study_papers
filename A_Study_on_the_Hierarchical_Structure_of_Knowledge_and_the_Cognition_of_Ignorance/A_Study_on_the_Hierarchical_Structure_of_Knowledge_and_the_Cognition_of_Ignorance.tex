...
\hypertarget{paper-scope-and-positioning}{%
\subsubsection{Paper Scope and
Positioning}\label{paper-scope-and-positioning}}

\textbf{What This Paper Is:}

This paper is a \textbf{conceptual framework paper}. Its primary
contributions are:
...
\tightlist
\item
  A \textbf{unifying observational schema} (the \(K_n\) family) that
  provides a common coordinate system for first-order accuracy,
  metacognitive alignment, and higher-order stability
\item
  A \textbf{taxonomic vocabulary} (the 27-pattern classification) for
  discussing metacognitive phenomena
\item
  A \textbf{principled separation} of epistemic state (\(K\)) from
  phenomenological confidence (\(C\))
\item
  \textbf{Illustrative derivations} showing how the framework relates to
  established metrics (IRT, meta-d') and \textbf{contrasts} with
  calibration metrics (ECE)
\end{enumerate}

\textbf{What This Paper Is NOT:}

This paper does \textbf{not} provide:
...
\item
  Complete probabilistic specifications of \(f_n\) and
  \(\text{State}_n\)
\item
  Estimation algorithms with likelihood functions and convergence
  guarantees
\item
  Empirical validation or simulation studies
\end{itemize}

These are valuable extensions that we explicitly designate as
\textbf{Future Work}.

\textbf{Rationale for This Scope:}

The metacognition literature currently lacks a unified conceptual
vocabulary that spans psychometrics (IRT), signal detection theory
(meta-d'), and machine learning calibration (ECE). Before developing
formal estimation theory or running experiments, the field needs
\textbf{settled conceptual foundations}. This paper provides those
foundations.

Formal measurement theory, estimation algorithms, and empirical
validation are planned for \textbf{separate technical papers} building
upon this conceptual scaffold.
...
\begin{quote}
\textbf{Note}: This is a \textbf{conceptual framework paper}. The
results below are illustrative derivations and informal propositions,
not rigorous theorems with complete proofs. See ``Paper Scope and
...
\(K_0 \approx \tanh(\theta)\) \\
\(K_1\) & Metacognitive alignment & meta-d'/d' (Sensitivity) &
...
\begin{quote}
\textbf{Note}: \(K_n\) measures \textbf{sensitivity} (discrimination
ability), NOT calibration bias. Calibration is captured by the
\((K, C)\) joint distribution.
\end{quote}
...
\hypertarget{technical-contributions-at-a-glance}{%
\subsubsection{Technical Contributions at a
Glance}\label{technical-contributions-at-a-glance}}

This section provides a roadmap to the \textbf{key results} in this
paper.

\begin{quote}
\textbf{Note}: As a conceptual framework paper, the results below are
\textbf{illustrative derivations} and \textbf{informal propositions},
not rigorous theorems with complete proofs. Formal identifiability
analysis connecting to general latent variable theory is deferred to
future technical work.
\end{quote}

\textbf{Results and Propositions:}
...
\bottomrule\noalign{}
\endlastfoot
\textbf{Result 1} & \(K_0\)-IRT Correspondence &
\(K_0 = \tanh(a(\theta-b)/2)\) (illustrative derivation) & Formal
Results \\
\textbf{Result 2} & \(K_1\)-Phi Correspondence & \(K_1 = \phi\) under
binary \(\text{State}_0/\text{Claim}_1\) & Formal Results \\
\textbf{Proposition 3} & \(K_0\) Identifiability (Informal) &
\(\text{Var}(b_i) > 0 \Rightarrow K_0\) identifiable & Formal Results \\
\textbf{Proposition 4} & \(K_1\) Identifiability (Informal) &
Variability conditions for \(K_1\) identification & Formal Results \\
\textbf{Proposition 5} & \(K_2\) Identifiability (Informal) & ICC-based
identifiability conditions & Formal Results \\
\textbf{Proposition 6} & Pipeline Identifiability (Informal) & Joint
identifiability of \((K_0, K_1, K_2)\) & Formal Results \\
\end{longtable}

\textbf{Falsifiable Predictions with Quantitative Bounds:}
...
\bottomrule\noalign{}
\endlastfoot
\textbf{Pred 1} & K-C Dissociation & \(\exists\) subjects with
\((K_1=-1, C=\text{high})\) & Axiom F \\
\textbf{Pred 2} & Layer Independence &
\(\text{Cor}(K_0, K_2 \mid K_1) \approx 0\) & Axiom F \\
\textbf{Pred 4} & Intervention Effect &
\(\mathbb{E}[\Delta K_1] \geq 0.2\),
\(|\mathbb{E}[\Delta K_0]| \leq 0.1\) & Axiom F \\
\textbf{Pred 5} & K-C Correlation Bound & \(\text{Cor}(K_1, C) < 0.85\)
& Axiom F \\
\end{longtable}

\textbf{Critical Clarifications:}

\begin{itemize}
\tightlist
\item
  \textbf{\(K_1 \approx \tanh(\text{meta-d}'/2)\)}: This is a
  \textbf{conceptual relationship}, NOT a mathematical identity (see
  Remark 1)
\item
  \textbf{\(\hat{K} = \text{identity}\)}: Justified by Lemma 3 under
  standard conditions
\item
  \textbf{Correspondences are illustrative derivations} under
  simplifying assumptions, not rigorous proofs: Result 1 provides
  derivation from 2PL-IRT under idealized conditions
\end{itemize}
...
\begin{quote}
\textbf{Critical Distinction: Abstention vs.~Ignorance}

The framework sharply distinguishes between two conceptually different
phenomena:

\begin{itemize}
\item
  \textbf{Abstention} (\(\text{State}_0\) = absent): A \emph{behavioral}
  observable---the subject chooses not to respond. This is measured at
  the object level (\(n = 0\)) as \(\text{Response}(x) = \emptyset\) and
  enters \(f_0\) as the third output category.
\item
  \textbf{Ignorance} (\(K_1 = -1\) with \(\text{State}_0\) = correct): A
  \emph{metacognitive} state---the subject responded correctly but
  believes they were guessing. This is a Type B error (unrecognized
  knowledge) measured at \(n = 1\).
\end{itemize}

The distinction matters because: 1. Abstention requires no response;
Ignorance requires a correct response with low confidence 2. Abstention
may reflect strategic behavior (risk aversion); Ignorance reflects
genuine metacognitive failure 3. The two patterns have different
educational implications: abstention may benefit from encouragement to
engage, while Type B ignorance requires confidence calibration

In forced-choice paradigms where abstention is not permitted, \(f_0\)
reduces to \(\{\text{correct}, \text{incorrect}\}\), and the framework
focuses purely on metacognitive accuracy. When abstention is permitted,
it provides additional diagnostic information about strategic epistemic
behavior.
\end{quote}
...
The coding of \(K_0 = -1\) (misconception) + ``I don't know'' \(\to\)
aligned (\(K_1 = +1\)) may appear counterintuitive. We justify this via
the \textbf{Epistemic Improvement Criterion}: a claim is ``aligned'' if
it represents the best available response given the subject's actual
state.
...
\(\to\) \(K_1 = 0.5\) (partial alignment) - \(K_0 = -1\), Claim = ``I
know'' \(\to\) \(K_1 = -1\) (full misalignment)
...
The coding of \(K_0 = -1\) (misconception) + ``I don't know'' \(\to\)
aligned (\(K_1 = +1\)) has been justified via the Epistemic Improvement
...
\textbf{HMeta-d (Fleming, 2017)} extends meta-d' to a hierarchical
Bayesian framework, enabling group-level inference and trial-by-trial
parameter estimation. The relationship to our framework is as follows:
...
\bottomrule\noalign{}
\endlastfoot
\textbf{Latent structure} & Single meta-d' per subject & Multi-layer
\((K_0, K_1, K_2)\) & HMeta-d \(\approx\) aggregate \(K_1\)
sensitivity \\
\textbf{Hierarchy} & Subjects nested in groups & Layers nested within
subjects & Orthogonal hierarchies \\
\textbf{Output} & meta-d'/d' ratio & Discrete \(K \in \{-1, 0, +1\}\) or
continuous \(K \in [-1, 1]\) &
\(K_1 \approx \tanh(\text{meta-d}'/2)\) \\
\textbf{Trial structure} & Binary (correct/incorrect by high/low
confidence) & Ternary (knowledge/ignorance/misconception) & K adds
misconception category \\
\textbf{Higher-order} & Not explicit (single meta level) & Explicit
recursive (\(K_2, K_3, \ldots\)) & K extends to arbitrary depth \\
\end{longtable}

\textbf{Integration Opportunity}: Researchers using HMeta-d can
incorporate the K framework by: 1. Using HMeta-d to estimate
subject-level meta-d' with shrinkage 2. Transforming to \(K_1\) via
\(\hat{K}_1 = \tanh(\widehat{\text{meta-d}'}/2)\) 3. Extending to
\(K_2\) via additional confidence judgments about Type-2 accuracy

\textbf{Reference}: Fleming, S. M. (2017). HMeta-d: Hierarchical
Bayesian estimation of metacognitive efficiency from confidence ratings.
\emph{Neuroscience of Consciousness, 3}(1), nix007.
...
\hypertarget{formal-results-illustrative-derivations-and-informal-propositions}{%
\subsection{Formal Results: Illustrative Derivations and Informal
Propositions}\label{formal-results-illustrative-derivations-and-informal-propositions}}

This section provides illustrative derivations and informal propositions
that establish the conceptual foundations of the \(K\) framework. These
results show how the framework relates to established metrics and
outline conditions under which key properties hold.

\begin{quote}
\textbf{Scope Note}: As stated in ``Paper Scope and Positioning,'' this
is a \textbf{conceptual framework paper}. The results below are
\emph{illustrative derivations} under idealized assumptions, not
rigorous theorems with complete proofs. Formal identifiability analysis
connecting to general latent variable theory (e.g., Allman et al., 2009;
Kruskal, 1977) is deferred to future technical work.
\end{quote}

\begin{quote}
\textbf{Summary of Contributions}: This section contains \textbf{2
results, 4 informal propositions, 1 lemma, and 5 falsifiable
predictions}. The main identifiability arguments are: -
\textbf{Proposition 3}: \(K_0\) identifiability under item variance
conditions - \textbf{Proposition 4}: \(K_1\) identifiability given
\(K_0\) and claim variability - \textbf{Proposition 6}: Joint
\((K_0, K_1, K_2)\) pipeline identifiability

For a condensed overview, see ``Technical Contributions at a Glance'' in
the Executive Summary.
\end{quote}

\begin{quote}
\textbf{Note on Contribution Type}: The results below primarily show how
the \(K\) framework connects to existing mathematical results (IRT,
Signal Detection Theory, ICC). The novelty lies not in the underlying
mathematics but in the unified conceptual integration that enables
systematic metacognition analysis.
\end{quote}

\hypertarget{core-results}{%
\subsubsection{Core Results}\label{core-results}}

\hypertarget{result-1-k_0-irt-correspondence}{%
\paragraph{\texorpdfstring{Result 1: \(K_0\)-IRT
Correspondence}{Result 1: K\_0-IRT Correspondence}}\label{result-1-k_0-irt-correspondence}}

\begin{quote}
\textbf{Note}: This is an \emph{illustrative derivation} under idealized
assumptions, not a general theorem.
\end{quote}

\textbf{Result 1} (\(K_0\)-IRT Correspondence):

\textbf{Assumptions} (Simplifying): - \textbf{(A1)} Item response
follows the 2-Parameter Logistic (2PL) IRT model:
\[P(\text{correct} | \theta, a_i, b_i) = \frac{1}{1 + e^{-a_i(\theta - b_i)}}\]
- \textbf{(A2)} \(K_0\) is defined as the signed expected accuracy:
\[K_0^{(i)} := 2P(\text{correct} | \theta, a_i, b_i) - 1\]

\textbf{Observation}: Under (A1)-(A2), for each item \(i\):
\[K_0^{(i)} = \tanh\left(\frac{a_i(\theta - b_i)}{2}\right)\]

\textbf{Derivation}: Starting from the 2PL probability:
...
\textbf{Scope of Validity}: This correspondence is exact under the
stated assumptions but does not constitute a general identifiability
result. Extension to realistic settings (polytomous items,
multidimensional traits, measurement error) requires additional formal
development.

\textbf{Status}: Illustrative; formal identifiability analysis deferred
to future work.

\textbf{Corollary 1.1} (Standardized Form): Under additional assumption
\textbf{(A3)}: \(a = 2\) and \(b = 0\): \[K_0 = \tanh(\theta)\]

\textbf{Corollary 1.2} (Aggregate Form): For \(N\) items with varying
\((a_i, b_i)\):
\[\bar{K}_0 = \frac{1}{N} \sum_{i=1}^{N} \tanh\left(\frac{a_i(\theta - b_i)}{2}\right)\]

\textbf{Boundary Condition} (Deviation from Standardization): When (A3)
is violated (i.e., \(a \neq 2\) or \(b \neq 0\)), the deviation from
\(\tanh(\theta)\) is:
...
\begin{longtable}[]{@{}
  >{\centering\arraybackslash}p{(\columnwidth - 10\tabcolsep) * \real{0.0641}}
  >{\centering\arraybackslash}p{(\columnwidth - 10\tabcolsep) * \real{0.0641}}
  >{\centering\arraybackslash}p{(\columnwidth - 10\tabcolsep) * \real{0.1282}}
  >{\centering\arraybackslash}p{(\columnwidth - 10\tabcolsep) * \real{0.3846}}
  >{\centering\arraybackslash}p{(\columnwidth - 10\tabcolsep) * \real{0.2179}}
...
\textbf{Interpretation}: The standardized form
\(K_0 \approx \tanh(\theta)\) is a good approximation only when item
discrimination \(a \approx 2\) and item difficulty \(b \approx 0\). For
items with extreme difficulty or low discrimination, the item-specific
form \(\tanh(a(\theta-b)/2)\) should be used.

\textbf{Scope of Validity}: - Holds exactly for binary correct/incorrect
responses - Does not hold for partial credit models (see Limitations) -
Assumes no guessing or slipping (see Q6 in Limitations)

\begin{center}\rule{0.5\linewidth}{0.5pt}\end{center}

\hypertarget{result-2-k_1-phi-correspondence}{%
\paragraph{\texorpdfstring{Result 2: \(K_1\)-Phi
Correspondence}{Result 2: K\_1-Phi Correspondence}}\label{result-2-k_1-phi-correspondence}}

\textbf{Observation}: When \(\text{State}_0\) and \(\text{Claim}_1\) are
both binary, \(K_1\) equals the Phi coefficient.

\textbf{Result 2} (\(K_1\)-Phi Correspondence): Let
\(\text{State}_0 \in \{\text{correct}, \text{incorrect}\}\) and
\(\text{Claim}_1 \in \{\text{"I know"}, \text{"I don't know"}\}\) be
binary random variables. Define
\(K_1 := \phi(\text{State}_0, \text{Claim}_1)\) where \(\phi\) is the
Phi coefficient:
\[\phi = \frac{n_{11} n_{00} - n_{10} n_{01}}{\sqrt{(n_{11}+n_{10})(n_{01}+n_{00})(n_{11}+n_{01})(n_{10}+n_{00})}}\]

Then: 1. \(K_1 \in [-1, 1]\) 2. \(K_1 = +1\) iff
\(\text{Claim}_1 = \text{State}_0\) for all items (perfect alignment) 3.
\(K_1 = -1\) iff \(\text{Claim}_1 = \neg\text{State}_0\) for all items
(perfect anti-alignment) 4. \(K_1 = 0\) iff
\(\text{Claim}_1 \perp \text{State}_0\) (statistical independence)
...
\begin{longtable}[]{@{}lcc@{}}
\toprule\noalign{}
& \(\text{Claim}_1\) = ``I know'' & \(\text{Claim}_1\) = ``I don't
know'' \\
\midrule\noalign{}
\endhead
\bottomrule\noalign{}
...
\textbf{Property 1} (\(K_1 \in [-1, 1]\)): The Phi coefficient is the
Pearson correlation for two binary variables. By the Cauchy-Schwarz
inequality, \(|\phi| \leq 1\).

\textbf{Property 2} (\(K_1 = +1\) iff perfect alignment): When
\(\text{Claim}_1 = \text{State}_0\) for all items,
\(n_{10} = n_{01} = 0\). Then:
\[\phi = \frac{n_{11} \cdot n_{00} - 0}{\sqrt{n_{11} \cdot n_{00} \cdot n_{11} \cdot n_{00}}} = \frac{n_{11} \cdot n_{00}}{n_{11} \cdot n_{00}} = 1\]

\textbf{Property 3} (\(K_1 = -1\) iff perfect anti-alignment): When
\(\text{Claim}_1 = \neg\text{State}_0\) for all items,
\(n_{11} = n_{00} = 0\). Then:
\[\phi = \frac{0 - n_{10} \cdot n_{01}}{\sqrt{n_{10} \cdot n_{01} \cdot n_{10} \cdot n_{01}}} = \frac{-n_{10} \cdot n_{01}}{n_{10} \cdot n_{01}} = -1\]

\textbf{Property 4} (\(K_1 = 0\) iff independence): Under statistical
independence, \(n_{ij} = n_{i\cdot} \cdot n_{\cdot j} / N\) for all
\(i, j\). Then:
...
\begin{quote}
\textbf{See Remark 1} (Appendix: Supplementary Propositions) for the
relationship between \(K_1\) and meta-d'. While
\(K_1 \approx \tanh(\text{meta-d}'/2)\) is used as a conceptual
heuristic in the Executive Summary, the two measures are \textbf{not
mathematically equivalent}---\(K_1\) is model-free and bounded, while
meta-d' depends on SDT assumptions and is unbounded. When SDT
assumptions are plausible, report both metrics; when questionable,
prefer \(K_1\).
\end{quote}

\textbf{Extension to Ternary \(\text{State}_0\)}: When
\(\text{State}_0 \in \{\text{correct}, \text{incorrect}, \text{absent}\}\),
binarization is required for Phi. The recommended strategy: - Positive:
correct only - Negative: incorrect + absent

This preserves the interpretation that ``I know'' should predict
correctness, not merely absence of misconception.

\begin{quote}
\textbf{Why Phi Coefficient Proves \(K_1\) is Pure Sensitivity}:

The Phi coefficient is a \textbf{correlation measure}. Correlation
captures \textbf{distributional shape} (co-variation between claims and
states), NOT \textbf{absolute value bias}.

A subject with systematic overconfidence (Bias) but perfect
discrimination (always higher confidence when correct than when
incorrect) will have \(K_1 = +1\).

This property proves that \(K_1\) is \textbf{Pure
Sensitivity}---independent of confidence magnitude bias. The bias
dimension is captured separately by \(C\) (Confidence).
\end{quote}

\begin{center}\rule{0.5\linewidth}{0.5pt}\end{center}

\hypertarget{identifiability-arguments-informal}{%
\subsubsection{Identifiability Arguments
(Informal)}\label{identifiability-arguments-informal}}

\begin{quote}
The following propositions outline \emph{informal arguments} for why the
\(K\) framework components should be identifiable under suitable
conditions. These are \textbf{not rigorous proofs}; formal
identifiability analysis connecting to general latent variable theory
(e.g., Allman et al., 2009; Kruskal, 1977) is deferred to future
technical work.
\end{quote}

\hypertarget{proposition-3-k_0-identifiability-informal}{%
\paragraph{\texorpdfstring{Proposition 3: \(K_0\) Identifiability
(Informal)}{Proposition 3: K\_0 Identifiability (Informal)}}\label{proposition-3-k_0-identifiability-informal}}

\textbf{Proposition 3} (\(K_0\) Identifiability): \(K_0\) is
identifiable from response data iff item difficulties have positive
variance.

Formally: Given \(N\) items with difficulties \(\{b_i\}_{i=1}^N\),
\(K_0\) (equivalently, \(\theta\)) is identifiable iff
\(\text{Var}(\{b_i\}) > 0\).
...
\textbf{Non-identifiability when \(\text{Var}(b_i) = 0\)}: If all
\(b_i = b\) (constant), define \(\theta' = \theta - c\) and
\(b' = b - c\) for any constant \(c\). Then:
\[a_i(\theta' - b') = a_i((\theta - c) - (b - c)) = a_i(\theta - b)\]
Thus \((\theta, b)\) and \((\theta', b')\) yield identical response
probabilities. The parameters are confounded.

\textbf{Identifiability when \(\text{Var}(b_i) > 0\)}: With varying
\(b_i\), the likelihood function has a unique maximum. Intuitively,
items with different difficulties ``anchor'' the scale: an easy item
(\(b_i\) low) and a hard item (\(b_i\) high) together locate \(\theta\)
uniquely. Formally, the Fisher information matrix is non-singular when
item difficulties vary (see Lord \& Novick, 1968, Theorem 17.3.1).
\(\blacksquare\)

\textbf{Reference}: Lord \& Novick (1968), \emph{Statistical Theories of
Mental Test Scores}, Chapter 17.

\textbf{Status}: Informal argument; formal proof requires connection to
general identifiability theory.

\begin{center}\rule{0.5\linewidth}{0.5pt}\end{center}

\hypertarget{lemma-3-hatk-sufficiency-and-non-uniqueness}{%
\paragraph{\texorpdfstring{Lemma 3: \(\hat{K}\) Sufficiency and
Non-Uniqueness}{Lemma 3: \textbackslash hat\{K\} Sufficiency and Non-Uniqueness}}\label{lemma-3-hatk-sufficiency-and-non-uniqueness}}

\begin{quote}
\textbf{Motivation}: A reviewer concern is whether the specific form of
\(\hat{K}\) is arbitrary. This lemma clarifies that \(\hat{K}\) is a
\emph{sufficient} but not \emph{unique} choice---any monotone,
anchor-preserving function yields equivalent ordinal results.
\end{quote}

\textbf{Lemma 3} (\(\hat{K}\) Sufficiency): Let \(\mathcal{K}\) be the
class of functions \(k: [0, 1] \to [-1, 1]\) satisfying: 1.
\textbf{Monotonicity}: \(c_1 > c_2 \Rightarrow k(c_1) \geq k(c_2)\) 2.
\textbf{Anchor Preservation}: \(k(0) = -1\), \(k(0.5) = 0\),
\(k(1) = +1\) 3. \textbf{Antisymmetry}: \(k(1-c) = -k(c)\) for all
\(c \in [0, 1]\)

\textbf{Statement}: All \(k \in \mathcal{K}\) yield identical ordinal
ranking of subjects and identical sign patterns for \(K_1\).

\textbf{Proof}: \emph{Step 1}: Any \(k \in \mathcal{K}\) can be written
as \(k(c) = \phi(2c - 1)\) where \(\phi: [-1, 1] \to [-1, 1]\) is
strictly increasing with \(\phi(0) = 0\).

\emph{Step 2}: For any two functions \(k_a, k_b \in \mathcal{K}\):
\[\text{sign}(k_a(c)) = \text{sign}(k_b(c)) = \text{sign}(2c - 1)\]

\emph{Step 3}: The ordinal structure---which subjects have higher
\(K_1\) than others---is preserved under any monotone transformation.
\(\blacksquare\)

\textbf{Corollary}: The specific functional form \(\hat{K}(c) = 2c - 1\)
is the \emph{simplest} member of \(\mathcal{K}\) but any member produces
equivalent results for: - Type A / Type B classification - Subject
ranking - Falsifiability tests - Practical interventions

The choice of the linear form is motivated by parsimony and
interpretability, not theoretical necessity.

\begin{center}\rule{0.5\linewidth}{0.5pt}\end{center}

\hypertarget{proposition-4-k_1-identifiability-informal}{%
\paragraph{\texorpdfstring{Proposition 4: \(K_1\) Identifiability
(Informal)}{Proposition 4: K\_1 Identifiability (Informal)}}\label{proposition-4-k_1-identifiability-informal}}

\textbf{Proposition 4} (\(K_1\) Identifiability): Given \(K_0\)
estimates and \(\text{Claim}_1\) observations, \(K_1\) is identifiable
iff: 1. \(P(K_0 = +1) > 0\) and \(P(K_0 \leq 0) > 0\) (variability in
\(\text{State}_0\)) 2. \(\text{Claim}_1\) is non-degenerate:
\(P(\text{Claim}_1 = \text{"I know"}) \in (0, 1)\)

\textbf{Informal Argument}: The Phi coefficient \(\phi\) is undefined
when any marginal frequency is zero (denominator becomes zero).
Condition 1 ensures both rows of the \(2 \times 2\) table are populated.
Condition 2 ensures both columns are populated. With all cells
potentially non-zero, \(\phi\) is a well-defined estimator of alignment.
\(\blacksquare\)

\textbf{Status}: Informal argument; formal proof deferred.

\begin{center}\rule{0.5\linewidth}{0.5pt}\end{center}

\hypertarget{proposition-5-k_2-identifiability-informal}{%
\paragraph{\texorpdfstring{Proposition 5: \(K_2\) Identifiability
(Informal)}{Proposition 5: K\_2 Identifiability (Informal)}}\label{proposition-5-k_2-identifiability-informal}}

\textbf{Proposition 5} (\(K_2\) Identifiability): Let
\(K_2 := \text{ICC}(K_1^{(t_1)}, K_1^{(t_2)})\) be the test-retest
intraclass correlation of \(K_1\).

\(K_2\) is identifiable iff: 1. \(\text{Var}(K_1) > 0\) (cross-subject
variability) 2. Measurement occasions \(t_1, t_2\) are sufficiently
separated (recommended: 2-4 weeks)

\textbf{Informal Argument}: The ICC is undefined when within-subject or
between-subject variance is zero. Condition 1 ensures between-subject
variance. The temporal separation in Condition 2 ensures that repeated
measurements are not autocorrelated beyond true stability.
\(\blacksquare\)

\textbf{Status}: Informal argument; formal proof deferred.

\hypertarget{proposition-6-recursive-pipeline-identifiability-informal}{%
\paragraph{Proposition 6: Recursive Pipeline Identifiability
(Informal)}\label{proposition-6-recursive-pipeline-identifiability-informal}}

\begin{quote}
\textbf{Note}: This proposition addresses the potential circularity
concern in recursive metacognition measurement---specifically, whether
measuring \(K_n\) might contaminate the \(K_{n+1}\) measurement,
creating an identification problem.
\end{quote}

\textbf{Proposition 6} (Pipeline Identifiability): Under the following
assumptions, the parameters \((K_0, K_1, K_2)\) are jointly identifiable
without circularity:

\textbf{Assumptions}: - \textbf{(A1) Reference Anchoring}: The
object-level ground truth \(\theta_0\) (item correctness) is fixed
externally before any elicitation. - \textbf{(A2) Temporal Precedence}:
\(\text{Claim}_n\) is elicited BEFORE any feedback about
\(\text{Claim}_{n-1}\) accuracy. - \textbf{(A3) Item Count Sufficiency}:
\(N \geq 10\) items per measurement occasion (for stable estimation). -
\textbf{(A4) Variance Conditions}: Non-zero between-subject and
within-subject variance in responses at each level.

\textbf{Statement}: Under (A1)-(A4), the mapping
\((\text{Response}_0, \text{Claim}_1, \text{Claim}_2) \mapsto (\hat{K}_0, \hat{K}_1, \hat{K}_2)\)
is injective for almost all parameter configurations, ensuring joint
identifiability.

\textbf{Informal Argument} (Four Steps):

\emph{Step 1: \(K_0\) Identification}.
\(K_0 = \mathbb{1}[\text{Response}_0 = \theta_0]\) is determined solely
by external ground truth comparison. By (A1), this requires no internal
reference and thus introduces no circularity.

\emph{Step 2: \(K_1\) Identification}. By (A2), \(\text{Claim}_1\) (the
subject's confidence in their response) is elicited before revealing
whether \(\text{Response}_0\) was correct. Thus:
\[K_1 = \text{sign}\left(\text{Claim}_1 - \frac{1}{2}\right) \cdot (2K_0 - 1)\]
The mapping from \((\text{Response}_0, \text{Claim}_1)\) to \(K_1\) uses
only \((K_0, \text{Claim}_1)\), where \(K_0\) is already identified in
Step 1.

\emph{Step 3: \(K_2\) Identification}. Similarly, \(\text{Claim}_2\)
(certainty about the correctness of \(\text{Claim}_1\)) is elicited
before feedback about \(\text{Claim}_1\) accuracy:
\[K_2 = \text{sign}\left(\text{Claim}_2 - \frac{1}{2}\right) \cdot (2|K_1| - 1)\]
By (A3) and (A4), sufficient items provide the variance structure needed
for stable estimation without contamination across levels.

\emph{Step 4: Injectivity}. Each level's identification depends only on:
(i) the immediately preceding level's already-identified parameter, and
(ii) independently elicited claims. The directed acyclic structure
\(K_0 \to K_1 \to K_2\) ensures no feedback loops. By Proposition 3
(conditional independence), the estimation errors are uncorrelated
across levels, establishing injectivity except on a measure-zero set of
degenerate configurations. \(\blacksquare\)

\textbf{Status}: Informal argument; connection to general
identifiability theory (Allman et al., 2009) deferred to future work.

\textbf{Remark} (Practical Implementation): The temporal precedence
requirement (A2) is satisfied by standard experimental protocols where
confidence judgments are collected before performance feedback.
Computer-based testing naturally enforces this separation.

\begin{center}\rule{0.5\linewidth}{0.5pt}\end{center}

\hypertarget{falsifiability-and-separation-axioms}{%
\subsubsection{Falsifiability and Separation
Axioms}\label{falsifiability-and-separation-axioms}}

\hypertarget{axiom-f-falsifiability}{%
\paragraph{Axiom F: Falsifiability}\label{axiom-f-falsifiability}}

\textbf{Axiom F} (Falsifiability): The \(K\) framework is empirically
falsifiable. We state five predictions whose violation would refute or
severely constrain the framework:

\textbf{Falsifiable Prediction 1} (K-C Dissociation):
...
If ALL subjects show \(K_1 = \text{sign}(2C - 1)\), the framework adds
nothing beyond confidence.

\textbf{Falsifiable Prediction 2} (Layer Independence):
\[\text{Cor}(K_0, K_2 \mid K_1) \approx 0\]

If \(K_2\) is strongly predicted by \(K_0\) after controlling for
\(K_1\), layer separation is violated.

\textbf{Falsifiable Prediction 3} (Intervention Sensitivity):
Metacognitive training should increase \(K_1\) while \(K_0\) may remain
constant.

If \(K_1\) cannot be moved independently of \(K_0\), the distinction is
spurious.

\textbf{Falsifiable Prediction 4} (Quantitative Bounds): ``Metacognitive
training improves \(K_1\) by at least \(\delta = 0.2\) on average, while
\(K_0\) remains within \(\varepsilon = 0.1\) of pre-training level.''

Formally: \[\mathbb{E}[K_{1,\text{post}} - K_{1,\text{pre}}] \geq 0.2\]
\[|\mathbb{E}[K_{0,\text{post}} - K_{0,\text{pre}}]| \leq 0.1\]

If training affects both \(K_0\) and \(K_1\) equally (i.e.,
\(\Delta K_1 \approx \Delta K_0\)), then layer separation is empirically
spurious for that intervention.

\textbf{Falsifiable Prediction 5} (K-C Correlation Bound): ``In any
population, \(\text{Cor}(K_1, C) < 0.85\).''

If \(\text{Cor}(K_1, C) > 0.85\) consistently across multiple datasets,
\(K_1\) and \(C\) are empirically redundant and the distinction adds no
value.

\begin{longtable}[]{@{}
  >{\raggedright\arraybackslash}p{(\columnwidth - 4\tabcolsep) * \real{0.2449}}
  >{\raggedright\arraybackslash}p{(\columnwidth - 4\tabcolsep) * \real{0.4898}}
...
\bottomrule\noalign{}
\endlastfoot
Pred 1 & All subjects show \(K_1 = \text{sign}(2C-1)\) & \(K\) reduces
to confidence \\
Pred 2 & \(\text{Cor}(K_0, K_2 \mid K_1) > 0.5\) & Layer hierarchy is
spurious \\
Pred 3 & \(\Delta K_1 = \Delta K_0\) under intervention & Levels not
separable \\
Pred 4 & \(\Delta K_1 < 0.2\) or \(|\Delta K_0| > 0.1\) & Bound
violation \\
...
\hypertarget{axiom-s-k-c-separation}{%
\paragraph{Axiom S: K-C Separation}\label{axiom-s-k-c-separation}}

\textbf{Axiom S} (K-C Separation): \(K\) (epistemic alignment) and \(C\)
(phenomenological confidence) are theoretically and operationally
distinct:

\textbf{Theoretical Distinction}: - \(K\): Alignment between claim and
actual state (requires feedback to compute) - \(C\): Subjective
certainty (measured before feedback is revealed)

\textbf{Operational Distinction}: - \(C\) is measured at \(t_1\) (during
response, before feedback) - \(K_1\) is computed at \(t_2\) (after
feedback reveals correctness)

\textbf{Empirical Separability Criterion}:
...
\bottomrule\noalign{}
\endlastfoot
\(\text{Cor}(K_1, C) < 0.5\) & Strongly separable & \(K_1\) captures
distinct construct \\
\(0.5 \leq \text{Cor}(K_1, C) < 0.7\) & Moderately separable & Both
constructs useful \\
\(0.7 \leq \text{Cor}(K_1, C) < 0.85\) & Weakly separable & Caution:
possible redundancy \\
\(\text{Cor}(K_1, C) \geq 0.85\) & Not separable & \(K_1\) adds no value
over \(C\) \\
\end{longtable}

\textbf{Note}: The 0.85 threshold corresponds to \(R^2 > 0.72\), meaning
more than 72\% of variance is shared.

\begin{center}\rule{0.5\linewidth}{0.5pt}\end{center}

\hypertarget{epistemic-logic-propositions}{%
\subsubsection{Epistemic Logic
Propositions}\label{epistemic-logic-propositions}}

\hypertarget{proposition-2-axiom-4-testability}{%
\paragraph{Proposition 2: Axiom 4
Testability}\label{proposition-2-axiom-4-testability}}

\textbf{Proposition 2} (Positive Introspection Testability): The S4/S5
Axiom 4 (positive introspection: \(Kp \to KKp\)) can be empirically
tested via the \(K\) framework.

\textbf{Statement}: In modal epistemic logic, Axiom 4 states: If an
agent knows \(p\), then they know that they know \(p\).

In the \(K\) framework, this corresponds to:
\(K_0 = +1 \Rightarrow K_1 = +1\) (idealized).

\textbf{Empirical Test}: Measure \(P(K_1 = +1 | K_0 = +1)\). If this
probability is less than 1, Axiom 4 is empirically violated.

\textbf{Axiom-4-Gap} (quantification of violation):
\[\text{Axiom-4-Gap} := 1 - P(K_1 = +1 | K_0 = +1)\]

A positive gap indicates systematic failure of positive introspection
(e.g., imposter syndrome).

\begin{center}\rule{0.5\linewidth}{0.5pt}\end{center}

\hypertarget{proposition-3-axiom-5-testability}{%
\paragraph{Proposition 3: Axiom 5
Testability}\label{proposition-3-axiom-5-testability}}

\textbf{Proposition 3} (Negative Introspection Testability): The S5
Axiom 5 (negative introspection: \(\neg Kp \to K\neg Kp\)) can be
empirically tested via the \(K\) framework.

\textbf{Statement}: In modal epistemic logic, Axiom 5 states: If an
agent does not know \(p\), then they know that they do not know \(p\).

In the \(K\) framework, this corresponds to:
\(K_0 \leq 0 \Rightarrow K_1 = +1\) (idealized Socratic wisdom).

\textbf{Empirical Test}: Measure \(P(K_1 = +1 | K_0 \leq 0)\). If this
probability is less than 1, Axiom 5 is empirically violated.

\textbf{Dunning-Kruger Index} (quantification of violation):
\[\text{DK-Index} := 1 - P(K_1 = +1 | K_0 \leq 0)\]

A positive DK-Index indicates systematic overconfidence among the
unknowing.

\begin{center}\rule{0.5\linewidth}{0.5pt}\end{center}

\hypertarget{measurement-propositions}{%
\subsubsection{Measurement
Propositions}\label{measurement-propositions}}

\hypertarget{proposition-4-incentive-compatibility}{%
\paragraph{Proposition 4: Incentive
Compatibility}\label{proposition-4-incentive-compatibility}}

\textbf{Proposition 4} (Incentive-Compatible Claim Elicitation): Under
Brier scoring, truthful probability reporting is the optimal strategy,
ensuring \(K_1\) reflects genuine calibration.

\textbf{Setup}: Elicit \(\text{Claim}_1\) as a probability
\(p \in [0, 1]\): ``What is the probability that your answer is
correct?''

Score using the Brier rule:
\[S(p, \text{outcome}) = 1 - (p - \mathbb{1}[\text{correct}])^2\]

\textbf{Statement}: The Brier score is a strictly proper scoring rule.
The expected score is maximized when
\(p = P(\text{correct} | \text{information})\).

\textbf{Implication for \(K_1\)}: When subjects are paid according to
Brier score, strategic responding is discouraged. The observed \(p\)
reflects genuine beliefs, ensuring that:
\[K_1 = 2 \cdot P(\text{Claim}_1 \text{ matches State}_0) - 1\] reflects
true metacognitive alignment rather than gaming behavior.

\textbf{Proof}: Properness of the Brier score is standard (Brier, 1950;
Gneiting \& Raftery, 2007). \(\blacksquare\)
...
\hypertarget{continuous-k_n-generation-model}{%
\subsubsection{\texorpdfstring{Continuous \(K_n\) Generation
Model}{Continuous K\_n Generation Model}}\label{continuous-k_n-generation-model}}

This section provides a complete specification of how continuous \(K_n\)
values are generated from latent variables, and how they relate to
discrete anchor categories.

\hypertarget{latent-variable-model}{%
\paragraph{Latent Variable Model}\label{latent-variable-model}}

\textbf{Definition} (Latent Alignment Strength): For each layer \(n\),
we posit a latent variable \(K_n^*\) representing the underlying
alignment strength:

\[K_n^* \sim \mathcal{N}(\mu_n, \sigma_n^2)\]

where: - \(\mu_n\): population mean alignment at layer \(n\) -
\(\sigma_n^2\): between-subject variance in alignment

\hypertarget{link-functions}{%
\paragraph{Link Functions}\label{link-functions}}

A \textbf{link function} \(h: \mathbb{R} \to [-1, 1]\) maps the
unbounded latent variable to the bounded \(K_n\) scale.

\textbf{Definition} (Link Function): \[K_n = h(K_n^*)\]

where \(h\) satisfies: - \(h(+\infty) = +1\) (anchor preservation at
positive extreme) - \(h(0) = 0\) (anchor preservation at zero) -
\(h(-\infty) = -1\) (anchor preservation at negative extreme) - \(h\) is
strictly monotonically increasing

\textbf{Available Link Functions}:
...
\bottomrule\noalign{}
\endlastfoot
\textbf{tanh} (default) & \(h(x) = \tanh(\beta x)\) & Smooth, symmetric,
S-shaped & General purpose; \(\beta\) controls sensitivity \\
\textbf{Scaled probit} & \(h(x) = 2\Phi(x) - 1\) & Probabilistic
interpretation & When latent variable represents log-odds \\
\textbf{Clipped linear} & \(h(x) = \max(-1, \min(1, x))\) & Simple,
piecewise linear & Quick approximation; not differentiable at
boundaries \\
\end{longtable}
...
The sensitivity parameter \(\beta > 0\) controls how quickly \(K_n\)
approaches the boundaries: - \(\beta < 1\): Compressed scale; more
values near 0 - \(\beta = 1\): Standard sensitivity - \(\beta > 1\):
Expanded scale; more values near \(\pm 1\)

\begin{quote}
\textbf{Rationale for tanh over logit}:

While the logistic function (logit link) is ubiquitous in IRT and binary
classification, we recommend \textbf{tanh as the default link} for the
following reasons:

\begin{enumerate}
\def\labelenumi{\arabic{enumi}.}
\item
  \textbf{Native range}: \(\tanh: \mathbb{R} \to (-1, 1)\) maps directly
  to the \(K\) scale without rescaling. The logistic
  \(\sigma: \mathbb{R} \to (0, 1)\) requires an affine transformation
  \(h(x) = 2\sigma(x) - 1\) to achieve the same range, introducing an
  extra step.
\item
  \textbf{Zero-centering}: \(\tanh(0) = 0\) naturally, placing the
  ignorance anchor at the origin. In contrast, \(\sigma(0) = 0.5\),
  requiring explicit centering.
\item
  \textbf{Symmetry}: \(\tanh(-x) = -\tanh(x)\), which mirrors the
  framework's symmetry between knowledge and misconception. The logistic
  function is not antisymmetric about zero without transformation.
\item
  \textbf{Gradient behavior}: The tanh gradient \(\text{sech}^2(x)\)
  peaks at \(x = 0\) (where discrimination matters most), whereas the
  logistic gradient peaks at \(\sigma = 0.5\) (mapped value \(0\)). For
  latent variable estimation, this distinction is minor, but tanh's
  direct form simplifies the algebra.
\end{enumerate}

\textbf{Empirical equivalence}: In practice,
\(\tanh(\beta x) = 2\sigma(2\beta x) - 1\), so results are
mathematically equivalent under appropriate parameter mapping. The
choice of tanh is thus a matter of \emph{notational convenience} and
\emph{conceptual clarity}, not theoretical necessity. Model fit
comparisons (AIC/BIC) should be identical up to reparameterization.
\end{quote}

\hypertarget{discretization-from-continuous-to-categorical}{%
\paragraph{Discretization: From Continuous to
Categorical}\label{discretization-from-continuous-to-categorical}}

\textbf{Definition} (Threshold-Based Discretization): Given thresholds
\(\tau^-\) and \(\tau^+\) with \(-1 < \tau^- < \tau^+ < 1\):
...
\bottomrule\noalign{}
\endlastfoot
\textbf{A: Equal-interval} (fixed) & \(\tau^+ = +\frac{1}{3}\),
\(\tau^- = -\frac{1}{3}\) & Divides \([-1,1]\) into three equal-length
intervals & Cross-study comparability; theoretical analysis \\
\textbf{B: Equal-frequency} (data-adaptive) & \(\tau^+ = Q_{67}(K_n)\),
\(\tau^- = Q_{33}(K_n)\) & Tercile-based; each category contains
\textasciitilde33\% of observations & Within-study analysis;
population-specific interpretation \\
\end{longtable}

\textbf{Terminology Note}: ``Tercile'' refers to percentile-based
cutoffs (Option B), not fixed values. Option A should be called
``equal-interval'' discretization.

\textbf{Selection Guideline}:
...
\textbf{Decision-Theoretic Justification for Option A}
(\(\tau = \pm 1/3\)): Under symmetric 0-1-2 loss (equal cost for
adjacent vs.~non-adjacent misclassification) and uniform prior on
\([-1, 1]\), the optimal decision boundaries are \(\pm 1/3\).

\hypertarget{point-estimation-and-uncertainty}{%
\paragraph{Point Estimation and
Uncertainty}\label{point-estimation-and-uncertainty}}

\textbf{Point Estimate}: For a subject with \(N\) items, the point
estimate of \(K_n\) is:
\[\hat{K}_n = \bar{K}_n = \frac{1}{N} \sum_{i=1}^{N} K_n^{(i)}\]

\textbf{Confidence Interval} (Bootstrap): 1. Resample \(N\) items with
replacement, \(B = 1000\) times 2. Compute \(\hat{K}_n^{(b)}\) for each
bootstrap sample \(b\) 3. Report 95\% CI:
\([\hat{K}_n^{(2.5\%)}, \hat{K}_n^{(97.5\%)}]\)

\textbf{Bayesian Posterior} (alternative): With prior
\(K_n^* \sim \mathcal{N}(0, \sigma_0^2)\) and likelihood based on
observed responses:
\[p(K_n^* | \text{data}) \propto p(\text{data} | K_n^*) \cdot p(K_n^*)\]

Report posterior mean and 95\% credible interval.

\textbf{Reliability Check}: If CI width \(> 0.4\), interpret the point
estimate with caution; consider increasing sample size or reducing
measurement noise.
...
High \(a\) (discriminating items) \(\to\) sharper transition near
\(\theta = b\) - High \(b\) (difficult items) \(\to\) shift toward lower
\(K_0\) for fixed \(\theta\)
...
\textbf{Monotonicity} & Increases with accuracy \(\times\) confidence &
Increases with metacognitive sensitivity \\
...
This section provides a \textbf{complete specification} of the protocol
for eliciting \(\text{Claim}_1\) and \(\text{Claim}_2\), and the
decision rules \(f_1\) and \(f_2\) for computing \(\text{State}_1\) and
\(\text{State}_2\).

\hypertarget{protocol-1-textclaim_1-elicitation-complete-specification}{%
\paragraph{\texorpdfstring{Protocol 1: \(\text{Claim}_1\) Elicitation
(Complete
Specification)}{Protocol 1: \textbackslash text\{Claim\}\_1 Elicitation (Complete Specification)}}\label{protocol-1-textclaim_1-elicitation-complete-specification}}

\textbf{Timing}: - \textbf{After} the subject's response to item \(x\) -
\textbf{Before} feedback on correctness is provided

\textbf{Instruction Template (Categorical)}: \textgreater{} ``How
confident are you that your answer is correct? \textgreater{} - {[} {]}
I'm confident I'm correct (''I know'') \textgreater{} - {[} {]} I'm not
sure (``Uncertain'') \textgreater{} - {[} {]} I think I might be wrong
(``I don't know'')''

\textbf{Instruction Template (Continuous)}: \textgreater{} ``What is the
probability that your answer is correct?'' \textgreater{} {[}Slider: 0\%
--------------------- 100\%{]}

\textbf{Mapping (Continuous \(\to\) Categorical)}:
...
\bottomrule\noalign{}
\endlastfoot
\textbf{Fixed thresholds} & \(p = 0.70\) and \(p = 0.30\) & Cross-study
comparability \\
\textbf{Pilot calibration} & Adjust to achieve \textasciitilde equal
category frequencies & Within-study optimization \\
\textbf{Percentile-based} & Top 30\%, middle 40\%, bottom 30\% &
Population-adaptive \\
\textbf{Domain-specific} & Higher thresholds for high-confidence domains
& Context sensitivity \\
\end{longtable}

\textbf{Inter-Rater Reliability Requirement}: - Categorical: Cohen's
\(\kappa > 0.80\) - Continuous: ICC \(> 0.70\)

\begin{center}\rule{0.5\linewidth}{0.5pt}\end{center}

\hypertarget{protocol-2-textclaim_2-elicitation-complete-specification}{%
\paragraph{\texorpdfstring{Protocol 2: \(\text{Claim}_2\) Elicitation
(Complete
Specification)}{Protocol 2: \textbackslash text\{Claim\}\_2 Elicitation (Complete Specification)}}\label{protocol-2-textclaim_2-elicitation-complete-specification}}

\textbf{Timing}: - \textbf{After} all items are completed -
\textbf{After} \(K_1\) is computed (or after metacognitive feedback is
provided)

\textbf{Instruction Template (Aggregate)}: \textgreater{} ``Thinking
about your self-assessments across all questions: \textgreater{} How
accurate do you think your self-assessments were? \textgreater{} - {[}
{]} My self-assessments were accurate (''Meta-aligned'') \textgreater{}
- {[} {]} I'm not sure about my self-assessments (``Meta-uncertain'')
\textgreater{} - {[} {]} My self-assessments were probably inaccurate
(``Meta-misaligned'')''

\textbf{Alternative: Item-Level \(\text{Claim}_2\)}: After revealing
\(K_1\) for each item: \textgreater{} ``Was your confidence judgment
appropriate for this item? \textgreater{} - {[} {]} Yes, appropriate
\textgreater{} - {[} {]} Not sure \textgreater{} - {[} {]} No,
inappropriate''

\begin{center}\rule{0.5\linewidth}{0.5pt}\end{center}

\hypertarget{protocol-3-f_1-decision-rules-complete-specification}{%
\paragraph{\texorpdfstring{Protocol 3: \(f_1\) Decision Rules (Complete
Specification)}{Protocol 3: f\_1 Decision Rules (Complete Specification)}}\label{protocol-3-f_1-decision-rules-complete-specification}}

\textbf{Input}: \((K_0, \text{Claim}_1)\) \textbf{Output}:
\(\text{State}_1 \in \{\text{aligned}, \text{uncertain}, \text{misaligned}\}\)

\textbf{Complete Decision Table}:
...
\begin{longtable}[]{@{}
  ...
  >{\raggedright\arraybackslash}p{(\columnwidth - 6\tabcolsep) * \real{0.3107}}
  >{\raggedright\arraybackslash}p{(\columnwidth - 6\tabcolsep) * \real{0.3398}}@{}}
\toprule\noalign{}
\begin{minipage}[b]{\linewidth}\raggedright
\(K_0\)
\end{minipage} & \begin{minipage}[b]{\linewidth}\raggedright
\(\text{Claim}_1\) = ``I know''
\end{minipage} & \begin{minipage}[b]{\linewidth}\raggedright
\(\text{Claim}_1\) = ``Uncertain''
\end{minipage} & \begin{minipage}[b]{\linewidth}\raggedright
\(\text{Claim}_1\) = ``I don't know''
\end{minipage} \\
\midrule\noalign{}
\endhead
\bottomrule\noalign{}
...
\textbf{*Edge Case Rationale}: \(K_0 = -1\) and \(\text{Claim}_1\) = ``I
don't know'' \(\to\) \textbf{aligned}

This coding follows the \textbf{Epistemic Improvement Criterion}: a
subject who has a misconception but recognizes they ``don't know'' is
exhibiting Socratic awareness, which is metacognitively appropriate.

\textbf{Alternative Interpretation (Stricter)}:
...
\bottomrule\noalign{}
\endlastfoot
\(-1\) & ``I don't know'' & \textbf{uncertain} (not aligned) \\
\end{longtable}

\textbf{Rationale for Alternative}: - ``I don't know'' does not
explicitly acknowledge misconception - True alignment at \(K_0 = -1\)
would require: ``I think I'm wrong''

\textbf{Framework Choice Guidelines}:
...
\bottomrule\noalign{}
\endlastfoot
\textbf{(a) Default (aligned)} & Recognition of non-knowledge (Socratic
wisdom) & General metacognition research \\
\textbf{(b) Alternative (uncertain)} & Recognition of specific error
direction & Error diagnosis, clinical settings \\
\end{longtable}

\textbf{Recommendation}: Always report which interpretation is used. For
replication, specify: ``We use interpretation (a)/(b) for the
\(K_0 = -1\), \(\text{Claim}_1\) = `I don't know' case.''
...
\hypertarget{protocol-4-f_2-decision-rules}{%
\paragraph{\texorpdfstring{Protocol 4: \(f_2\) Decision
Rules}{Protocol 4: f\_2 Decision Rules}}\label{protocol-4-f_2-decision-rules}}

\textbf{Input}: \((K_1, \text{Claim}_2)\) \textbf{Output}:
\(\text{State}_2 \in \{\text{meta-aligned}, \text{meta-uncertain}, \text{meta-misaligned}\}\)
...
\(K_1\) (aggregate)
\end{minipage} & \begin{minipage}[b]{\linewidth}\raggedright
\(\text{Claim}_2\) = ``Accurate''
\end{minipage} & \begin{minipage}[b]{\linewidth}\raggedright
\(\text{Claim}_2\) = ``Uncertain''
\end{minipage} & \begin{minipage}[b]{\linewidth}\raggedright
\(\text{Claim}_2\) = ``Inaccurate''
\end{minipage} \\
\midrule\noalign{}
\endhead
\bottomrule\noalign{}
...
\hypertarget{continuous-claim-variants}{%
\paragraph{Continuous Claim Variants}\label{continuous-claim-variants}}
...
\hypertarget{relationship-to-polytomous-item-response-theory}{%
\subsubsection{Relationship to Polytomous Item Response
Theory}\label{relationship-to-polytomous-item-response-theory}}

The K framework draws on and extends classical Item Response Theory
(IRT). This section clarifies the relationship to polytomous IRT
models---particularly the Graded Response Model (GRM; Samejima, 1969)
and the Generalized Partial Credit Model (GPCM; Muraki, 1992).

\hypertarget{background-polytomous-irt-models}{%
\paragraph{Background: Polytomous IRT
Models}\label{background-polytomous-irt-models}}
...
\bottomrule\noalign{}
\endlastfoot
\textbf{GRM} (Graded Response) & Ordered categories (e.g., 0-4) &
Cumulative probability structure & Attitude scales, rubric scoring \\
\textbf{GPCM} (Generalized Partial Credit) & Partial credit categories &
Adjacent-category logits & Multi-step problem solving \\
\textbf{RSM} (Rating Scale) & Common thresholds & Constrained GPCM &
Likert scales \\
\end{longtable}

\hypertarget{structural-correspondence}{%
\paragraph{Structural Correspondence}\label{structural-correspondence}}
...
\toprule\noalign{}
\begin{minipage}[b]{\linewidth}\raggedright
K Framework Component
\end{minipage} & \begin{minipage}[b]{\linewidth}\raggedright
Polytomous IRT Analog
\end{minipage} & \begin{minipage}[b]{\linewidth}\raggedright
Correspondence
\end{minipage} \\
\midrule\noalign{}
...
\bottomrule\noalign{}
\endlastfoot
\(K_0 \in \{-1, 0, +1\}\) (discrete) & GRM categories \(\{0, 1, 2\}\) &
Isomorphic under linear transformation \\
\(K^*\) (latent variable) & \(\theta\) (ability parameter) & Identical
interpretation as latent trait \\
Link function \(h(K^*)\) & Item characteristic curve & Both map latent
to observable \\
Between-layer independence & Dimensional structure & Multidimensional
IRT with layer-specific traits \\
\end{longtable}

\textbf{Formal Relationship}:

The K framework can be re-expressed as a \textbf{multidimensional GRM}
where each metacognitive layer defines a separate latent dimension:

\[P(K_{n,i} \geq k | \theta_n) = \frac{1}{1 + \exp(-a_{n,i}(\theta_n - b_{n,k}))}\]

where: - \(\theta_n\): Layer-specific latent trait (metacognitive
ability at level \(n\)) - \(a_{n,i}\): Discrimination parameter for item
\(i\) at layer \(n\) - \(b_{n,k}\): Threshold for category \(k\) at
layer \(n\)

\hypertarget{key-differences}{%
\paragraph{Key Differences}\label{key-differences}}

Despite the structural similarity, the K framework differs from standard
polytomous IRT in three respects:

\begin{enumerate}
\def\labelenumi{\arabic{enumi}.}
\item
  \textbf{Semantic Anchoring}: GRM/GPCM categories are typically ordinal
  without intrinsic meaning. The K scale has fixed semantic anchors:
  \(-1\) = misconception, \(0\) = ignorance, \(+1\) = knowledge. This
  enables cross-domain comparison.
\item
  \textbf{Recursive Structure}: Standard IRT treats dimensions as
  parallel or hierarchical. The K framework imposes a specific recursive
  dependency: \(K_{n+1}\) evaluates the accuracy of \(K_n\), creating an
  explicit epistemological structure absent in generic multidimensional
  IRT.
\item
  \textbf{Negative Values}: The K scale extends to \([-1, 1]\) to model
  misconception explicitly. Standard GRM uses \([0, M]\) where \(0\) is
  the lowest category. The \(K = -1\) anchor (confident but wrong)
  requires this extension.
\end{enumerate}

\hypertarget{implementation-guidance}{%
\paragraph{Implementation Guidance}\label{implementation-guidance}}

For researchers implementing the K framework within existing IRT
software:
...
\endlastfoot
\textbf{mirt} (R) & Use
\texttt{mirt(...,\ itemtype\ =\ \textquotesingle{}graded\textquotesingle{})}
with rescaled responses \\
\textbf{ltm} (R) & \texttt{grm()} function with manual post-hoc
transformation to \([-1, 1]\) \\
\textbf{Stan} & Custom GRM with explicit prior on threshold ordering \\
\textbf{TAM} (R) & \texttt{tam.mml()} with appropriate scoring matrix \\
\end{longtable}

\textbf{Transformation}: Given GRM responses \(Y \in \{0, 1, 2\}\), the
K-scale transformation is \(K = Y - 1\).
...
correct \\
\textbf{\(K_1\)} & Metacognitive alignment & meta-d' (Sensitivity) &
...
\begin{quote}
\textbf{Important}: In this framework, \textbf{Metacognitive Alignment
(\(K_1\))} is formally equivalent to \textbf{Metacognitive Sensitivity}
(discrimination ability). This represents pure discrimination ability,
independent of the subject's confidence magnitude (Bias).
\end{quote}
...
\textbf{Why \(K\) and \(C\) are Decoupled (Not \(K\) and ECE):}

This framework explicitly \textbf{decouples} sensitivity from
calibration bias:
...
\bottomrule\noalign{}
\endlastfoot
\textbf{\(K_n\)} & Sensitivity (discrimination ability) & Absolute
confidence bias \\
\textbf{\(C\)} & Confidence magnitude (bias) & Discrimination ability \\
\textbf{\((K, C)\) joint} & Full metacognitive profile & N/A \\
\end{longtable}

\begin{quote}
\textbf{Key insight}: A subject with \(K_1 = +1\) (perfect sensitivity)
may still be severely miscalibrated if their \(C\) distribution is
biased. Calibration is diagnosed from \((K, C)\) discrepancy, not from
\(K\) alone.
\end{quote}
...
\textbf{\(K_1\)} & This framework & Alignment direction (sensitivity) \\
\textbf{\(K_2\)} & This framework & Meta-metacognitive stability \\
\textbf{\(C\)} & This framework & Confidence magnitude (bias
dimension) \\
...
sensitivity structure, \(C\) for bias dimension.
...
\hypertarget{correspondence-with-monitor-generate-verify-architectures}{%
\paragraph{Correspondence with Monitor-Generate-Verify
Architectures}\label{correspondence-with-monitor-generate-verify-architectures}}

Recent LLM metacognition research formalizes recursive self-monitoring
via Monitor-Generate-Verify (MGV) loops. The \(K\) framework provides a
natural coordinate system for these architectures.
...
In MGV, the loop proceeds: 1. Generate response \(\to\) observe \(K_0\)
(against reference) 2. Monitor quality \(\to\) compute \(K_1\)
(self-assessment alignment) 3. Verify monitoring \(\to\) compute \(K_2\)
...
\hypertarget{formal-contributions-summary}{%
\subsubsection{Formal Contributions
Summary}\label{formal-contributions-summary}}

The paper establishes the following results (detailed in ``Formal
Results: Illustrative Derivations and Informal Propositions''):
...
\(K_0 = \tanh(a(\theta-b)/2)\) (illustrative derivation) & Formal
Results \\
\textbf{Result 2} & \(K_1\)-Phi correspondence under binary conditions &
Formal Results \\
\textbf{Proposition 3} & \(K_0\) identifiability:
\(\text{Var}(b_i) > 0 \Rightarrow K_0\) identifiable (informal) & Formal
Results \\
\textbf{Proposition 4} & \(K_1\) identifiability given \(K_0\) and claim
variability (informal) & Formal Results \\
\textbf{Proposition 5} & ICC reliability conditions for stable \(K\)
measurement (informal) & Formal Results \\
\textbf{Proposition 6} & Pipeline identifiability: \((K_0, K_1, K_2)\)
jointly identifiable (informal) & Formal Results \\
\textbf{Lemma 3} & \(\hat{K}\) sufficiency: any monotone
anchor-preserving function yields equivalent ordinal results & Formal
Results \\
\end{longtable}

\textbf{Key Identifiability Arguments (Informal)}: - The framework
outlines conditions under which parameters should be recoverable from
data - The recursive measurement pipeline avoids circularity by design
(Proposition 6) - The specific form of \(\hat{K}\) is sufficient but not
unique (Lemma 3)

\textbf{Falsifiable Predictions}: Five quantitative predictions (P1-P5)
with explicit bounds enable empirical refutation of the framework.
...
\item
  ...
  \tightlist
  \item
    ``I'm 80\% confident'' matches \(K_0 = 0.6\) \(\to\) partial
    alignment
  \item
    ``I'm 80\% confident'' matches \(K_0 = -0.2\) \(\to\) partial
    misalignment
  \end{itemize}
...
\item
  ...
  but not developed here. \textbf{This is explicitly designated as
  Future Work.} The current paper provides the conceptual framework;
  LLM-specific operationalizations (token-level probability extraction,
  multi-sample self-consistency, debate protocols) require separate
  empirical validation.
\item
  \textbf{Normative vs Descriptive Interpretation:}

  \textbf{The Tension}: Recent work (e.g., positive evidence bias
  literature) suggests that some ``metacognitive biases'' may be
  rational responses to high-dimensional hypothesis spaces, not
  failures.

  \textbf{Our Position}: The \(K\) framework is \textbf{DESCRIPTIVE},
  not normative.

  \(K_1 = -1\) means: ``The subject's claim does not match their actual
  state relative to the experimenter-defined reference.''

  This is a \textbf{COORDINATE}, not a \textbf{JUDGMENT}.

  \textbf{When \(K_1 = -1\) Might Be ``Rational''}:
  ...
  Scenario
  \end{minipage} & \begin{minipage}[b]{\linewidth}\raggedright
  Why \(K_1 = -1\) May Be Rational
  \end{minipage} \\
  \midrule\noalign{}
  \endhead
  \bottomrule\noalign{}
  \endlastfoot
  Contested reference & Experimenter's ``ground truth'' is incomplete or
  biased \\
  Different objective & Subject optimizes for social harmony, not
  accuracy \\
  Information asymmetry & Subject has valid information experimenter
  lacks \\
  Bayesian conservatism & Prior knowledge rationally weighted against
  weak evidence \\
  \end{longtable}

  \textbf{Recommendation}: Report \(K_1\) alongside task context. Do not
  interpret \(K_1 = -1\) as ``failure'' without considering whether the
  reference standard is appropriate.

  \textbf{Methodological Relativism (Reiterated)}: The framework does
  NOT adjudicate what is ``correct.'' \(K_1 = -1\) is descriptive of a
  state, not prescriptive of a norm.
\end{enumerate}
...
\hypertarget{future-work-technical-extensions}{%
\subsubsection{Future Work: Technical
Extensions}\label{future-work-technical-extensions}}

This conceptual framework invites several technical extensions that are
beyond the current paper's scope:

\textbf{1. Formal Measurement Theory} - Probabilistic specification of
\(f_n\) and \(\text{State}_n\) as random variables - Likelihood-based
estimation with noise models - Identifiability conditions connected to
general latent variable theory (Allman et al., 2009; Anandkumar et al.,
2014)

\textbf{2. Estimation Algorithms} - EM or MCMC algorithms for continuous
\(K_n\) estimation - Handling of abstentions and missing data (MNAR
modeling) - Computational complexity and convergence analysis

\textbf{3. Empirical Validation} - Synthetic experiments with known
ground-truth \(K_n\) profiles - Recovery of \(K_0\), \(K_1\) under
noise; testing correspondence to IRT/meta-d'/ECE - Stress tests for K-C
dissociation and layer independence predictions

\textbf{4. Extended Metrics} - Integration with improved calibration
metrics (ACE, SCE, class-wise calibration) - Connections to Hierarchical
meta-d' (Fleming, 2017) estimation

\textbf{5. Applications} - LLM metacognition operationalization -
Educational diagnostics - Clinical assessment protocols

We view these as \textbf{separate technical contributions} that build
upon the conceptual scaffold established here.
...
\hypertarget{why-k-times-c-joint-model-instead-of-splitting-k_1}{%
\subsubsection{\texorpdfstring{Why \(K \times C\) Joint Model Instead of
Splitting
\(K_1\)}{Why K \textbackslash times C Joint Model Instead of Splitting K\_1}}\label{why-k-times-c-joint-model-instead-of-splitting-k_1}}

One reviewer suggested splitting \(K_1\) into separate dimensions:
\(K_1^{\text{sens}}\) (sensitivity) and \(K_1^{\text{cal}}\)
(calibration), analogous to how meta-d' and ECE measure different
aspects of metacognition.

\textbf{We argue that our \((K, C)\) joint model provides a superior
solution.}

\begin{longtable}[]{@{}
  >{\raggedright\arraybackslash}p{(\columnwidth - 6\tabcolsep) * \real{0.1724}}
  >{\raggedright\arraybackslash}p{(\columnwidth - 6\tabcolsep) * \real{0.3621}}
...
\bottomrule\noalign{}
\endlastfoot
\textbf{Split \(K_1\)} & \(K_1^{\text{sens}} + K_1^{\text{cal}}\) &
Directly maps to meta-d'/ECE & Destroys recursive unity of \(K\) \\
\textbf{Our \((K, C)\) joint} & \(K\) (sensitivity) + \(C\) (bias) &
Preserves recursion; explicit phenomenological dimension & Requires
bivariate analysis \\
\end{longtable}

\textbf{Design Rationale:}

Existing approaches treat sensitivity (meta-d') and calibration (ECE) as
separate metrics. This framework \textbf{unifies} them differently:

\begin{itemize}
\tightlist
\item
  \textbf{\(K_n\) dimension}: Sensitivity (discrimination ability) ---
  bias-independent
\item
  \textbf{\(C\) dimension}: Confidence magnitude (bias) ---
  sensitivity-independent
\item
  \textbf{\((K, C)\) joint model}: Captures complex phenomena like
  Dunning-Kruger
\end{itemize}

This preserves the recursive unity of the \(K\) operator
(\(K_0 \to K_1 \to K_2 \to \cdots\)) while maintaining orthogonal
dimensions for structural state (\(K\)) and phenomenological feeling
(\(C\)).
...
\bottomrule\noalign{}
\endlastfoot
Dunning-Kruger & \(K_1 = -1\) (anti-correlation) & \(C = \text{High}\) &
Misalignment + overconfidence \\
Imposter Syndrome & \(K_1 = -1\) (anti-correlation) & \(C = \text{Low}\)
& Misalignment + underconfidence \\
Calibrated Expert & \(K_1 = +1\) (alignment) & \(C \propto K_0\) &
Alignment + appropriate confidence \\
\end{longtable}

The \((K, C)\) representation is no less expressive than splitting
\(K_1\), but retains the elegant recursive structure.

\hypertarget{appendix-technical-lemmas-and-supplementary-propositions}{%
\subsection{Appendix: Technical Lemmas and Supplementary
Propositions}\label{appendix-technical-lemmas-and-supplementary-propositions}}

\hypertarget{robustness-lemmas}{%
\subsubsection{Robustness Lemmas}\label{robustness-lemmas}}

\hypertarget{lemma-1-scale-invariance-of-k_0}{%
\paragraph{\texorpdfstring{Lemma 1: Scale Invariance of
\(K_0\)}{Lemma 1: Scale Invariance of K\_0}}\label{lemma-1-scale-invariance-of-k_0}}

\textbf{Lemma 1} (Scale Invariance): \(K_0 = \tanh(a(\theta - b)/2)\) is
invariant under affine rescaling of \(\theta\) iff \(a\) and \(b\) are
co-transformed.

Formally: Let \(\theta' = c\theta + d\) for constants \(c > 0\) and
\(d\). Then \(K_0(\theta') = K_0(\theta)\) iff:
...
\textbf{Implication}: \(K_0\) values are robust to arbitrary
scale/location transformations of the latent ability \(\theta\),
provided item parameters are appropriately re-estimated.

\begin{center}\rule{0.5\linewidth}{0.5pt}\end{center}

\hypertarget{lemma-2-k_1-under-class-imbalance}{%
\paragraph{\texorpdfstring{Lemma 2: \(K_1\) under Class
Imbalance}{Lemma 2: K\_1 under Class Imbalance}}\label{lemma-2-k_1-under-class-imbalance}}

\textbf{Lemma 2} (Class Imbalance Bound): Let \(\pi = P(K_0 = +1)\) be
the base rate of correct knowledge.

The Phi coefficient \(K_1\) satisfies:
\[|K_1| \leq \sqrt{\frac{\min(\pi, 1-\pi)}{\max(\pi, 1-\pi)}}\]

\textbf{Corollary}: When \(\pi \to 0\) or \(\pi \to 1\), \(|K_1| \to 0\)
regardless of true alignment.

\textbf{Proof}:

The Phi coefficient achieves its maximum when the \(2 \times 2\) table
has deterministic structure. Given marginal constraint
\(\pi = P(K_0 = +1)\), the maximum occurs when: - If \(\pi \leq 0.5\):
All \(K_0 = +1\) cases have \(\text{Claim}_1\) = ``I know'', and the
remaining ``I know'' claims are minimized. - The resulting maximum is:
\[|\phi|_{\max} = \sqrt{\frac{\min(\pi, 1-\pi)}{\max(\pi, 1-\pi)}}\]

\textbf{Derivation}: Let \(p = P(\text{Claim}_1 = \text{"I know"})\).
The maximum correlation between two binary variables with marginals
\(\pi\) and \(p\) is achieved when they are monotonically related. By
the Frechet-Hoeffding bounds, the maximum joint probability is
\(\min(\pi, p)\), yielding:
\[\phi_{\max} = \frac{\min(\pi, p) - \pi p}{\sqrt{\pi(1-\pi)p(1-p)}}\]

Optimizing over \(p\) and applying algebra gives the stated bound. As
\(\pi \to 0\) or \(\pi \to 1\), the bound \(\to 0\). \(\blacksquare\)

\textbf{Recommendation}: Report \(K_1\) alongside \(\pi\) (base rate).
For robustness checks, compute Matthews Correlation Coefficient (MCC)
which is equivalent to Phi but emphasizes this constraint in
interpretation.

\begin{center}\rule{0.5\linewidth}{0.5pt}\end{center}

\hypertarget{supplementary-propositions}{%
\subsubsection{Supplementary
Propositions}\label{supplementary-propositions}}

\hypertarget{remark-1-k_1-and-meta-dcomplementary-perspectives}{%
\paragraph{\texorpdfstring{Remark 1: \(K_1\) and meta-d'---Complementary
Perspectives}{Remark 1: K\_1 and meta-d'---Complementary Perspectives}}\label{remark-1-k_1-and-meta-dcomplementary-perspectives}}

\begin{quote}
\textbf{Critical Note}: The relationship between \(K_1\) and meta-d' is
\textbf{conceptual}, not mathematical. We do \textbf{NOT} claim
\(K_1 \approx \tanh(\text{meta-d}'/2)\) as an identity or conversion
formula. These measures capture related but distinct aspects of
metacognition.
\end{quote}

\textbf{Remark 1} (\(K_1\) and meta-d': Complementary Perspectives):

\(K_1\) and meta-d' (Maniscalco \& Lau, 2012) measure related but
distinct aspects of metacognition:

\textbf{meta-d'} (Signal Detection Theory): -
\textbf{Sensitivity-based}: How well does confidence discriminate
correct from incorrect responses? - Rooted in Signal Detection Theory
(SDT) - Requires Gaussian assumptions on evidence distributions - Unit:
d' scale (unbounded, typically in range \([0, 4]\)) - Definition:
\(\text{meta-d}' = \Phi^{-1}(\text{HR}_2) - \Phi^{-1}(\text{FAR}_2)\)

\textbf{\(K_1\)} (This framework): - \textbf{Alignment-based}: Does the
subject's claim match their actual epistemic state? - Assumption-free
when using Phi coefficient - Bounded: \(K_1 \in [-1, 1]\) - Definition:
\(K_1 = \phi(\text{State}_0, \text{Claim}_1)\)

\textbf{Relationship}: - Both are high when metacognition is accurate -
Both are low/zero when confidence is unrelated to accuracy - However,
they are \textbf{not mathematically equivalent}

\textbf{Why No Exact Correspondence}: 1. meta-d' is defined via inverse
normal CDFs; \(K_1\) via correlation 2. meta-d' depends on SDT model
assumptions; \(K_1\) is model-free 3. meta-d' is unbounded; \(K_1\) is
bounded to \([-1, 1]\)

\textbf{Empirical Expectation}: - In well-calibrated populations with
Gaussian confidence distributions:
\[\text{Cor}(K_1, \tanh(\text{meta-d}'/2)) > 0.6\] (expected but not
guaranteed) - In populations with non-Gaussian distributions or extreme
base rates: divergence is expected

\textbf{Recommendation}: - Report \textbf{both} \(K_1\) and meta-d' when
SDT framework assumptions are plausible - Prefer \(K_1\) when SDT
assumptions are questionable (e.g., non-binary responses, non-Gaussian
confidence) - Do \textbf{not} use
\(K_1 \approx \tanh(\text{meta-d}'/2)\) as an identity or conversion
formula

\textbf{Complementary Use Case}: \textbar{} Question \textbar{}
Preferred Metric \textbar{}
\textbar:---------\textbar:-----------------\textbar{} \textbar{} ``How
well does confidence separate correct from incorrect?'' \textbar{}
meta-d' \textbar{} \textbar{} ``Does the subject's claim align with
their actual state?'' \textbar{} \(K_1\) \textbar{} \textbar{} ``What is
the pattern of metacognitive (mis)alignment per item?'' \textbar{}
\(K_1^{(i)}\) (item-level) \textbar{}

\begin{center}\rule{0.5\linewidth}{0.5pt}\end{center}

\begin{quote}
\textbf{Note on Calibration Metrics (ECE)}:

While widely used metrics like ECE (Expected Calibration Error)
\textbf{conflate} sensitivity and calibration bias, this framework
\textbf{decouples} them:

\begin{itemize}
\tightlist
\item
  \textbf{\(K_n\)} captures the \textbf{sensitivity component}
  (discrimination ability)
\item
  The \textbf{\((K, C)\) joint distribution} reveals the
  \textbf{calibration bias}
\end{itemize}

ECE is NOT approximated by \(K_1\). Instead, calibration error is
diagnosed by examining the discrepancy between \(K\) values and \(C\)
values: - High \(K_1\) + systematically high \(C\) -\textgreater{}
Overconfidence bias - High \(K_1\) + systematically low \(C\)
-\textgreater{} Underconfidence bias

This separation preserves the recursive unity of the \(K\) operator
while maintaining orthogonal dimensions for state (\(K\)) and feeling
(\(C\)).
\end{quote}

\begin{center}\rule{0.5\linewidth}{0.5pt}\end{center}

\hypertarget{claim-1-framework-novelty}{%
\paragraph{Claim 1: Framework Novelty}\label{claim-1-framework-novelty}}

\textbf{Claim 1} (Framework Novelty---Survey-Based): Based on our survey
of existing approaches, the \(K\) framework is the only approach
satisfying ALL of the following properties:

\begin{quote}
\textbf{Note}: This is a survey-based claim, not a mathematically
provable proposition. The uniqueness is established relative to the
approaches reviewed (meta-d', ECE/Brier, IRT), not as an absolute
logical necessity.
\end{quote}

\begin{longtable}[]{@{}
  >{\raggedright\arraybackslash}p{(\columnwidth - 8\tabcolsep) * \real{0.2000}}
  >{\centering\arraybackslash}p{(\columnwidth - 8\tabcolsep) * \real{0.3000}}
...
\bottomrule\noalign{}
\endlastfoot
\textbf{(a)} Per-item metacognitive classification & Yes & No & No &
Yes \\
\textbf{(b)} Explicit modeling of ignorance (\(K=0\)) distinct from
misconception (\(K=-1\)) & Yes & No & No & No \\
\textbf{(c)} Recursive higher-order extension (\(K_2, K_3, ...\)) & Yes
& No & No & No \\
\textbf{(d)} Unified anchor semantics across layers & Yes & N/A & N/A &
N/A \\
\end{longtable}

\textbf{Justification (per property)}:

\textbf{(a) Per-item metacognitive classification}: - meta-d': Computes
aggregate sensitivity across all items; cannot classify individual items
as ``Socratic'' or ``Dunning-Kruger'' - ECE/Brier: Aggregate calibration
metrics; no item-level output - IRT: Provides per-item
difficulty/discrimination but no metacognitive layer - \textbf{\(K\)
framework}: \(K_1^{(i)}\) is defined for each item \(i\)

\textbf{(b) Explicit ignorance (\(K=0\)) vs misconception (\(K=-1\))}: -
meta-d': Binary Type-1 outcome (correct/incorrect); ``I don't know''
treated as low confidence, not distinct state - ECE/Brier: No
representation of ``absence of stance'' - IRT: \(\theta\) is continuous;
no categorical ``ignorance'' state - \textbf{\(K\) framework}:
\(K_0 = 0\) explicitly represents epistemic absence, distinct from
\(K_0 = -1\) (wrong belief)

\textbf{(c) Recursive higher-order extension}: - meta-d': Single-layer
(Type-2); no meta-meta-d' defined in literature - ECE/Brier: No
recursive structure - IRT: First-order only; no \(\theta(\theta)\) -
\textbf{\(K\) framework}: \(K_2, K_3, \ldots\) formally defined with
consistent semantics

\textbf{(d) Unified anchor semantics}: - This property is unique to
recursive frameworks; meta-d'/ECE/IRT have no analogous requirement -
\textbf{\(K\) framework}: Same anchors (\(+1, 0, -1\)) with consistent
interpretation across all layers

\textbf{Conclusion}: No existing framework satisfies (a)-(d)
simultaneously. \(\blacksquare\)

\begin{center}\rule{0.5\linewidth}{0.5pt}\end{center}

\hypertarget{proposition-6-computational-complexity}{%
\paragraph{Proposition 6: Computational
Complexity}\label{proposition-6-computational-complexity}}

\textbf{Proposition 6} (Polynomial-Time Estimation): All \(K_n\)
estimates (\(n = 0, 1, 2\)) can be computed in polynomial time.
...
\bottomrule\noalign{}
\endlastfoot
\(K_0\) & 2PL-IRT via EM algorithm & \(O(N \cdot I \cdot \text{iter})\)
where \(N\) = subjects, \(I\) = items \\
\(K_1\) & Phi coefficient & \(O(N)\) \\
\(K_2\) & ICC (two-way random effects) & \(O(N \cdot T)\) where \(T\) =
measurement occasions \\
\end{longtable}

All operations are standard and available in common statistical software
(R: \texttt{ltm}, \texttt{psych}; Python: \texttt{pymc},
\texttt{scipy}).

\begin{center}\rule{0.5\linewidth}{0.5pt}\end{center}

\hypertarget{worked-example-complete-k_2-computation}{%
\subsubsection{\texorpdfstring{Worked Example: Complete \(K_2\)
Computation}{Worked Example: Complete K\_2 Computation}}\label{worked-example-complete-k_2-computation}}

This section provides a concrete, step-by-step example of computing
\(K_0\), \(K_1\), and \(K_2\) for a single subject responding to 10
items.

\hypertarget{step-1-raw-data-collection}{%
\paragraph{Step 1: Raw Data
Collection}\label{step-1-raw-data-collection}}

A subject completes 10 multiple-choice items with \(\text{Claim}_1\)
elicitation:
...
\hypertarget{step-2-compute-k_0-per-item}{%
\paragraph{\texorpdfstring{Step 2: Compute \(K_0\) per
Item}{Step 2: Compute K\_0 per Item}}\label{step-2-compute-k_0-per-item}}

Using the embedding \(g_0\): correct \(\to\) +1, abstain \(\to\) 0,
incorrect \(\to\) -1:
...
\textbf{Aggregate \(K_0\)}:
\(\bar{K}_0 = \frac{1+1-1+0+1-1-1+1+0+1}{10} = \frac{2}{10} = 0.2\)

\hypertarget{step-3-compute-k_1-per-item}{%
\paragraph{\texorpdfstring{Step 3: Compute \(K_1\) per
Item}{Step 3: Compute K\_1 per Item}}\label{step-3-compute-k_1-per-item}}
...
*Item 3: \(K_0 = -1\) with ``I don't know'' \(\to\) aligned (Epistemic
Improvement Criterion)

\textbf{Aggregate \(K_1\)}:
\(\bar{K}_1 = \frac{1+1+1+1+1-1+0+1+1+1}{10} = \frac{7}{10} = 0.7\)

\hypertarget{step-4-elicit-textclaim_2}{%
\paragraph{\texorpdfstring{Step 4: Elicit
\(\text{Claim}_2\)}{Step 4: Elicit \textbackslash text\{Claim\}\_2}}\label{step-4-elicit-textclaim_2}}

After completing all items, subject is asked: \textgreater{} ``How
accurate do you think your self-assessments were?''

Subject responds: \textbf{``My self-assessments were accurate''} \(\to\)
\(\text{Claim}_2\) = ``Meta-aligned''

\hypertarget{step-5-compute-k_2}{%
\paragraph{\texorpdfstring{Step 5: Compute
\(K_2\)}{Step 5: Compute K\_2}}\label{step-5-compute-k_2}}

\textbf{Option A: Threshold-Based}

Discretize \(\bar{K}_1 = 0.7\) using equal-interval thresholds
(\(\tau^+ = 1/3\), \(\tau^- = -1/3\)): - \(\bar{K}_1 = 0.7 > 1/3\)
\(\to\) Discrete \(K_1\) = +1 (mostly aligned)

Compare with \(\text{Claim}_2\): - Claimed: ``Meta-aligned'' (expects
alignment) - Actual: Discrete \(K_1\) = +1 (aligned) - Match?
\textbf{Yes} \(\to\) \(\text{State}_2\) = meta-aligned \(\to\)
\(K_2 = +1\)

\textbf{Option B: Test-Retest ICC}
...
\hypertarget{step-6-summary}{%
\paragraph{Step 6: Summary}\label{step-6-summary}}
...
\(K_2\) (Option A) & +1 & Subject correctly recognizes their good
metacognition \\
\(K_2\) (Option B) & 0.82 & Stable metacognition across time \\
\end{longtable}

\textbf{Pattern Classification} (using \((K_0, K_1, K_2) = (0, +1, +1)\)
discretized): - Pattern \#14: \textbf{Socratic Wisdom}---Subject has
limited knowledge but accurately recognizes this and knows their
...
\item
  Muraki, E. (1992). A generalized partial credit model: Application of
  an EM algorithm. \emph{Applied Psychological Measurement},
  \emph{16}(2), 159-176.
\item
  Embretson, S. E., \& Reise, S. P. (2000). \emph{Item Response Theory
  for Psychologists}. Lawrence Erlbaum Associates.
\item
  Fleming, S. M. (2017). HMeta-d: Hierarchical Bayesian estimation of
  metacognitive efficiency from confidence ratings. \emph{Neuroscience
  of Consciousness}, \emph{3}(1), nix007.
\item
